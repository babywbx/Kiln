...
安装脚本带 `--service` 时会注册一个 systemd unit；只有 `/etc/kiln/kiln.toml` 已存在时才同时启用并启动。这一步需要 root，且只支持带 systemd 的 Linux。
...
<Aside type="note" title="旧 EPG 缓存不会迁移">
  旧版 `epg.cache_dir` 里的 `<sha256>.cache` 文件不会导入新的 `epg.db`。升级后首次启动会重新抓取已启用的节目单源，成功前节目单可能为空。确认 `epg.db` 已刷新后再清理旧文件；需要回退时可先保留。
</Aside>
...
<CardGrid>
  <Card title="/healthz" icon="ph:heartbeat">
    存活探针。只要服务监听器在跑就返回 `200` 与 `{"status":"ok"}`，不检查任何依赖。适合做进程守护和容器重启判定。
  </Card>
...
该子命令使用系统信任库并正常校验证书，没有跳过校验或自定义 CA 参数。自动生成的自签名证书尚未受信时，HTTPS 探测会失败；分离监听时优先探测明文健康端口。镜像里已经配好 `HEALTHCHECK`：`core` 与 `full` 用 `wget --no-check-certificate` 先试 HTTPS 再回退 HTTP；`lite` 基于 `scratch`，直接用上面这个子命令探测 HTTP。监听与证书细节见 [HTTPS 与分离监听](/guide/https/)。
