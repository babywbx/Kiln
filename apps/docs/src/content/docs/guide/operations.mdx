---
title: 运维
description: 以服务方式长期运行 Kiln：systemd 与 Windows 服务、数据目录与备份、升级、日志、健康检查、指标、OTLP 追踪、pprof 与资源自适应。
sidebar:
  order: 10
---

这一页覆盖 Kiln 上线之后的日常事务：怎么把它挂成系统服务，数据放在哪里、怎么备份，升级要注意什么，以及日志、健康检查、指标、追踪和资源自适应各自如何配置与核对。
...
备份最省事的做法是停掉进程再整目录复制。在线备份务必把 `kiln.db-wal` 一起带上，只拷主库会丢掉尚未 checkpoint 的写入。`sessions/` 可以直接跳过，它在每次启动新会话时都会被清空重建。
...
级别解析接受若干别名：`dbg` 与 `trace` 等同于 `debug`，`warning`、`wrn` 等同于 `warn`，`err`、`erro`、`fatal` 等同于 `error`；无法识别的值一律回落到 `info`。格式只区分两种，`structured` 是 `json` 的别名，其余值都按 `text` 处理。
...
<CardGrid>
  ...
  <Card title="/readyz" icon="ph:check-circle">
    就绪探针。除了存活，还会检查兼容引擎：当目录里存在 `ingress = "dash"` 且生效引擎为 `ffmpeg` 的频道时，若 ffmpeg 不可用则返回 `503` 与 `not_ready`，消息为 `ffmpeg compatibility engine is not available`。适合做流量摘挂。
  </Card>
</CardGrid>
...
端点由 `[observe].enabled` 控制，而配置加载时会把它置为开启，因此 `core` 与 `full` 始终对外提供 `/metrics`。`lite` 不注册这个路由。
...
填了 `[observe].otlp_endpoint` 才会初始化导出器，留空时完全不引入追踪开销。
...
- 有效 milli-CPU 小于 4000 时，流水线上限取 `ceil(milli / 1000)`；4 核起不再下压。
...
<Aside type="caution" title="只会下压，不会上调">
...
`effective_memory_mb` 是探测结果，`memory_limit_mb` 是最终写入的 Go 软目标，`effective_go_memory_limit_mb` 是运行时实际生效的值。三者对不上时，先看有没有设 `GOMEMLIMIT`。若 `resource_profile` 显示 `configured`，说明内存探测没拿到数值，档位判定被跳过了，这时用 `KILN_RESOURCE_MEMORY_MB` 手工指定。
...
<CardGrid>
  <Card title="故障排查" icon="ph:first-aid-kit">
    症状对照表、真实日志关键字与处理步骤，见[故障排查](/guide/troubleshooting/)。
  </Card>
  <Card title="配置参考" icon="ph:sliders">
    每个配置段的完整字段与默认值，见[配置参考](/reference/config/)。
  </Card>
  <Card title="环境变量" icon="ph:terminal-window">
    所有 `KILN_` 前缀变量与优先级，见[环境变量](/reference/env/)。
  </Card>
</CardGrid>
