---
title: 鉴权与凭据
description: Kiln 的四种凭据、会话 JWT 密钥管理、管理员 API Token 与生产加固清单。
sidebar:
  order: 4
---

Kiln 把「谁在调用」拆成四种互不重叠的凭据：公开端点不需要凭据，登录会话服务管理控制台，管理员 API Token 服务脚本与自动化，路径式播放密钥服务播放器分发。任何一种都无法把自己升级成另一种。
...
| 管理员 API Token | `kiln_v1_` 前缀 + 48 位随机字符 | 脚本、CI、外部管理工具 | 创建时设定有效期，可轮换、可撤销 |
...
- 签发的令牌带 `iss`、`aud`、`exp`、`iat`、`nbf`、`sub` 和一次性 `jti`；校验时四项时间断言全部生效，允许 30 秒时钟偏差。
- `token_issuer` 与 `token_audience` 默认都是 `kiln`，校验时必须与签发时一致。改了这两项，历史令牌全部作废。
- `token_ttl_hours` 默认 24，取值小于等于 0 时回落到 24 小时。
...
会话 JWT 是给浏览器的，脚本不该拿它。长期自动化请在管理控制台的「设置 → 管理员 API Token」里签发独立凭据。
...
每一次使用管理员 API Token 的请求都会落一条记录，允许和拒绝都记：Token 前缀、方法与路径、所需权限、判定结果与拒绝原因、HTTP 状态、客户端地址、User-Agent 和请求编号。控制台在同一张卡片下方展示最近的调用记录。
...
<Aside type="danger" title="Token 无法自我提权">
  管理员 API Token 不能修改登录凭据，也不能创建、编辑、轮换其他 Token 或读取 Token 审计日志：这些路由只认登录会话。更重要的是，服务端维护一张显式的路由权限登记表，**未登记的路由一律 403**，新增接口不会因为遗漏而默认对 Token 开放。
</Aside>
...
| `allowed_hosts` | 出站白名单，限制 Kiln 允许拉取的源站主机。配置中已声明的上游与频道源地址主机会自动并入，通常只需补充重定向落到的额外域名 |
...
除此之外，所有响应都会带上 `X-Content-Type-Options`、`Referrer-Policy`、`X-Frame-Options: DENY`、严格的 `Content-Security-Policy` 与 `Cache-Control: no-store`，管理控制台因此不能被嵌入到第三方页面里。
...
<Steps>
  ...
  <Step title="收紧主机白名单">
    用 `public_hosts` 限定对外域名，用 `allowed_hosts` 限定允许拉取的源站。两者都留空意味着任何 `Host` 都能访问，且出站范围完全由频道配置决定。
  </Step>
  <Step title="给自动化独立 Token">
    脚本一律使用管理员 API Token，按用途授予最小权限并设置有效期，定期轮换，通过审计日志核对调用来源。
  </Step>
</Steps>
