...
Kiln 使用四类凭据，各自只负责一类访问：公开端点无需凭据，会话与预览 JWT 用于管理控制台和受保护播放，管理员 API 令牌用于脚本和自动化，路径式播放密钥用于播放器和播放列表分发。不同凭据的权限互不继承，也不能相互转换。
...
| 会话或预览 JWT | `Authorization: Bearer <jwt>` 或 `?token=`，Ed25519 签名 | 管理控制台、受保护的播放列表与 `/v1/play/*` | 会话默认 24 小时；预览令牌 5 分钟且只限单个频道 |
...
| 键 | 作用 |
| --- | --- |
| `play_require_auth` | `/v1/playlist.m3u` 与 `/v1/play/` 是否需要凭据，默认开启。关闭后两组地址都对任何人可用，仅供本机调试；环境变量 `KILN_PLAY_OPEN=1` 是同一个开关 |
