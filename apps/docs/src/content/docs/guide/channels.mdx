---
title: 频道与上游
description: 频道模型逐字段说明、上游定义、配置与控制台的关系，以及启停、导入导出与自检操作。
sidebar:
  order: 2
---

频道是 Kiln 里唯一的核心对象：它把一个上游地址、一种入口格式和一套运行策略绑在一起，对外表现为一条播放地址和播放列表里的一行。本页逐字段讲清楚这个模型。
...
<Aside type="note" title="启动时的自动拉起">
  进程启动阶段的自动拉起以配置文件里声明的频道为准。只在管理控制台创建的频道，请用「立即启动」或第一个观众来拉起会话。
</Aside>
...
- `prefer_height`：目标分辨率高度。为 0 或留空时回落到 `[ffmpeg].prefer_height`。
- `preferred_audio_languages`：按优先级排列的音轨语言代码数组，例如 `["zh", "en"]`。
- `selection`：更精确的轨道选择，可分别对视频、音频与字幕指定模式和具体轨道。详见 [变体与轨道](/guide/variants/)。
- `packager`：频道级引擎覆盖，取值同 `[packager].engine`；填了非法值会被忽略，回落到全局设置。
...
- `proxy`：固定走某条出站线路，留空时按 [出站代理](/guide/proxy/) 的规则决定。
...
- **频道**：配置文件里的 `[[channels]]` 只在首次启动、频道表为空时被写入数据库。此后数据库就是唯一权威来源，改配置文件不会回灌，也不会覆盖控制台里的改动。要批量调整已有频道，用控制台或管理接口。
...
- **出站线路与 EPG 源**：同样是首次启动播种、之后以数据库为准。
...
- 停用只是让频道对外消失，不会删除频道定义，也不会影响它在列表里的排序。
- 正在运行的会话不会因为停用而立刻消失，需要时用 `DELETE /v1/admin/sessions/{id}` 显式停掉。
