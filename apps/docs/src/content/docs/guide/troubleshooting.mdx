...
**症状**：保存频道的代理绑定时被拒绝，消息为 `FFmpeg cannot use a SOCKS proxy; choose HTTP, direct, or the native DASH engine`；或者会话启动时报 `CONNECT tunnel cannot use socks5 proxy`。

**判断**：兼容引擎的全部网络请求会先经过 Kiln 的本地安全代理，再接入频道的出站线路。这个转接只支持 `http` 与 `https` 两种 scheme。原生引擎则四种都支持（`http`、`https`、`socks5`、`socks5h`）。
