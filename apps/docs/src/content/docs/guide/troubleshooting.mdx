---
title: 故障排查
description: 按症状定位 Kiln 的常见问题：启动失败、播放鉴权、频道起不来、播放停滞、内存偏高、EPG 不刷新、代理不生效与容器场景。
sidebar:
  order: 11
---

这一页按「症状 → 判断 → 处理」组织。每一节都给出可以直接 grep 的真实日志关键字和接口返回消息，照着对就行。运维相关的配置项说明在[运维](/guide/operations/)。
...
先确认用的是哪一类凭据。四种凭据各管一段，混用是最常见的原因：公开接口不需要凭据，会话 JWT 服务管理界面，管理员 API Token 服务脚本，路径式播放密钥服务播放器分发。详见[鉴权](/guide/auth/)。
...
| `403` | `API token permission denied` | 管理员 API Token 缺少该路由要求的 scope，或者这条路由只允许会话登录访问 |

### 凭据类型用错

**症状**：拿管理员 API Token 去调 `/v1/play/...`，或者拿会话 JWT 去访问 `/p/<token>/...`。
...
**判断**：`security.play_require_auth` 的默认值是开启。只有设置了环境变量 `KILN_PLAY_OPEN` 时，配置文件里写的 `play_require_auth = false` 才会被保留；否则配置加载阶段会把它强制置回 `true`。

**处理**：调试环境用 `KILN_PLAY_OPEN=1` 关闭播放鉴权。
...
**判断**：出站请求会做 SSRF 校验。公网主机默认放行，但回环地址和私有网段必须出现在允许列表里；云元数据地址（例如 `169.254.169.254`）和链路本地地址一律拒绝。允许列表由 `security.allowed_hosts` 加上配置文件里 `upstreams` 与 `channels` 声明过的主机名共同组成。

**处理**：把内网源站的主机名或 IP 写进 `security.allowed_hosts`，然后重启。通过管理界面新增的内网上游不会自动进入这个集合。
...
<Steps>
  <Step title="EPG 总开关">
    `[epg].enabled` 必须为 `true`。`lite` 变体不支持 EPG，配置里开了会直接拒绝启动。
  </Step>
  <Step title="源是否启用">
    内置源默认全部停用，需要在管理界面或配置里逐个启用。启动日志的 `epg_sources` 字段就是当前生效的源数量，为 `0` 说明一个都没启用。
  </Step>
  <Step title="体积超限">
  ...
  <Step title="代理与网络">
    `EPG refresh failed` 的 `err` 是连接超时或 TLS 错误时，说明源本身拉不通。每个 EPG 源可以单独指定 `proxy`，留空或写 `auto` 表示按全局出站规则路由。
  </Step>
</Steps>
...
**判断**：路由决策是「按优先级升序遍历规则，第一条命中就采用，都不命中则回落到 `egress.default`」。也就是说 `priority` 数值越小越先被检查。规则种类有五种：`host_exact`、`host_suffix`（默认）、`host_regex`、`channel_id`、`url_regex`。
...
**判断**：命中的规则指向了一个不存在或已停用的线路时，决策会静默降级为直连，`reason` 变成 `host_suffix:example.com|missing-profile`。启动时如果 `egress.default` 指向未知线路，会直接报 `egress.default references unknown proxy "x"` 并退出。

**处理**：检查 `[[proxies]]` 里的 `id` 是否拼写一致，以及该线路是否被停用。

### ffmpeg 用不了 SOCKS 代理

**症状**：保存频道的代理绑定时被拒绝，消息为 `FFmpeg cannot use a SOCKS proxy; choose HTTP, direct, or the native DASH engine`；或者会话启动时报 `proxy "x" uses socks5, which ffmpeg cannot use`。

**判断**：兼容引擎是通过 `HTTP_PROXY` 系列环境变量把代理传给 ffmpeg 的，只支持 `http` 与 `https` 两种 scheme。原生引擎则四种都支持（`http`、`https`、`socks5`、`socks5h`）。

**处理**：给该频道换成 HTTP 代理，或者改用原生引擎。

### 容器里连不上宿主机代理

**症状**：Kiln 跑在容器里、代理跑在宿主机上，兼容引擎报连接失败。

**判断**：把代理写成 `http://127.0.0.1:7890` 时，容器内的 `127.0.0.1` 指向容器自己。`[ffmpeg].mode = "docker"` 下，Kiln 会自动把代理地址里的 `127.0.0.1`、`localhost`、`::1` 替换成 `egress.docker_proxy_host`（默认 `host.docker.internal`），端口保持不变。传给 ffmpeg 的环境变量里还会带上 `NO_PROXY=localhost,127.0.0.1`。

**处理**：确认 `docker_proxy_host` 在当前环境下可解析。Linux 上的 Docker 需要显式加 `--add-host=host.docker.internal:host-gateway`，或者把它改成宿主机在 docker 网桥上的地址。
...
**判断**：`configured` 意味着内存探测没有拿到有效数值，档位判定被整个跳过了。探测顺序是 cgroup v2 的 `memory.max`、cgroup v1 的 `memory.limit_in_bytes`，最后是 `/proc/meminfo` 的 `MemTotal`，取其中最小的正值。嵌套 cgroup、非常规挂载布局或者没有设内存限制时，可能拿不到期望值。
