...
<Aside type="tip" title="先看两条日志">
  绝大多数问题都能从启动日志判断出来。`listening` 会显示计划监听的地址与 `scheme`；分离模式会分别出现 HTTP 与 HTTPS 两条，随后仍可能因端口占用而失败。`kiln starting` 那一条把版本、配置路径、频道数、资源档位、`packager_engine`、`ffmpeg_available`、代理数量和数据库路径全打出来了。先确认这些记录存在，再往下排查。
</Aside>
...
先确认正在使用哪一类凭据。四种凭据各有用途，混用是最常见的原因：公开接口不需要凭据，会话与预览 JWT 用于管理界面和受保护播放，管理员 API 令牌用于脚本，路径式播放密钥用于向播放器分发。详见 [鉴权](/guide/auth/)。
...
<Aside type="danger" title="不要在对外服务上关闭播放鉴权">
  关掉之后任何人都能取得全量播放列表并直接拉流。这是调试开关，不是部署选项。
</Aside>
...
如果日志显示 HTTPS 源重定向到公网主机的 `http://...:80`，可按 [频道管理](/guide/channels/#修复错误的降级重定向) 为该上游或直接 URL 频道开启 `upgrade_insecure_redirects`。它只处理这种错误的重定向，不会修复普通网络、DNS 或 TLS 证书问题。
...
| `packager.fetch_stall_sec` | 30 秒 | 响应正文连续没有任何新字节，覆盖清单、分片和错误正文；有字节进展就重新计时，不是总下载时长 |
| `packager.stall_timeout_sec` | 180 秒 | 清单仍在刷新，但所有轨道一直没有发布新分片；`-1` 可关闭 |
...
**处理**：磁盘库默认是 `data_dir/epg/epg.db`。先修复网络、代理或体积上限并手动刷新；只有确认数据库损坏时，才停服务、把 `epg` 目录移到备份位置后重启。旧版 `<sha256>.cache` 不会被新版导入。刷新间隔由 `epg.refresh_interval_min` 控制，默认 360 分钟，改小之后要注意源站的速率限制。详见 [EPG](/guide/epg/)。
...
**判断**：HTTP、HTTPS 与 `socks5h` 线路会把原始域名交给代理解析，`socks5` 则由 Kiln 所在主机先解析。无论选哪条线路，Kiln 都会在连接代理前执行本地 DNS 与 SSRF 校验；`probe target dns lookup failed` 或 `resolved to a non-public address` 因此可能在代理接管前出现。

**处理**：优先让 Kiln 使用能返回真实地址的 DNS，或改用可信的 HTTP(S)／`socks5h` 线路。只有确实可信的内网源才把精确主机名或 IP 加入 `security.allowed_hosts`，不要用通配方式放开私网校验。
...
如果手工 `-healthcheck` 指向自动生成的自签名 HTTPS，在证书进入系统信任库前会失败。分离监听优先检查明文端口，HTTPS-only 则使用已受信的证书或上面的镜像探测方式，详见 [HTTPS 与分离监听](/guide/https/)。
