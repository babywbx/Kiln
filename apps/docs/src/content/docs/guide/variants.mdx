---
title: Core、Full 与 Lite
description: 三个发行变体的能力边界、构建与分发形态、Lite 的固定资源预算、受控实测对比数据与选型指引。
sidebar:
  order: 6
---

Kiln 发行三个变体。它们共用同一套配置模型和同一份原生媒体模块，区别在于控制面的完整度、是否携带 ffmpeg，以及资源预算的松紧。

## 三个变体

| 维度 | Core | Full | Lite |
| --- | --- | --- | --- |
| 默认 packager | `native` | `auto` | `native` |
| 原生 HLS / DASH | 是 | 是 | 是 |
| FFmpeg | 不包含 | 内置 8.1.2 | 不包含 |
| 登录、M3U、播放 | 是 | 是 | 是 |
| SQLite 数据库 | 是 | 是 | 否 |
| 管理控制台与管理 API | 是 | 是 | 否 |
| EPG | 是 | 是 | 否 |
| OTLP 与 pprof | 是 | 是 | 否 |
| 基础镜像 | Alpine | Alpine 加 FFmpeg | `scratch` |
| 定位 | 完整的纯原生部署 | 完整的兼容部署 | 固定配置的低资源播放节点 |
