...
抓取到的 XMLTV 会边解析边写入一个 SQLite 库，节目单本身不常驻内存，查询时按匹配频道按需读取。`cache` 与 `cache_dir` 一起决定这个库放在哪里，共三种组合。

<Tabs>
  <TabItem label="磁盘（默认）">
    `cache = true`，`cache_dir` 指向一个目录，缺省为 `{data_dir}/epg`，库文件是其中的 `epg.db`。

    库跨重启存活，抓取时会带上 `If-None-Match` 与 `If-Modified-Since`，源没有变化时直接命中 304 并跳过整轮解析。抓取或解析失败时，上一轮已入库的内容继续对外提供，并把该源标记为过期。占用的磁盘大致与源解压后的体积相当。

    旧版写入的 `<sha256>.cache` 原始 XMLTV 文件不会导入 `epg.db`。升级后首次启动会重新完整抓取，成功前对应节目单可能为空；确认新库刷新成功后再清理旧文件，需回退旧版时可继续保留它们。
  </TabItem>
  <TabItem label="内存">
    `cache = true`，且 `cache_dir = "memory"`（`:memory:` 等价）。

    条件请求与失败回退的行为与磁盘一致，但不产生任何磁盘写入，进程重启后需要重新完整下载一遍。适合只读根文件系统，或不希望在数据目录里堆积大文件的部署。内存占用仍会随源文件规模增长，但不会再额外保留一份完整的解析对象图。
  </TabItem>
  <TabItem label="关闭">
    `cache = false`。

    不落盘，改用进程内的临时库，也就没有跨重启的条件请求与回退。此外，每次访问 `/v1/epg.xml` 或 `/v1/epg.xml.gz` 都会先同步刷新一遍全部源，再渲染响应。
  </TabItem>
</Tabs>
...
- 下载超限时本轮抓取失败，已有数据保持可用并被标记为过期，不会被半截内容覆盖。超限在流式读取过程中就会中断，不会先把整份文档读进内存。
- 在 `auto` 模式下，这个值也会随可用内存降低：内存低于 256 MiB 时降到 4 MiB，其余低资源档位按内存量在 4 MiB 到 64 MiB 之间取值。磁盘模式下，它主要限制下载与数据库体积，而不再对应一份常驻的 Go 对象图。
