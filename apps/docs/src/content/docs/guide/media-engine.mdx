---
title: 媒体引擎
description: engine 的三种取值、原生 DASH 解密重封装的能力边界、密钥文件、LL-HLS 参数、内存与延迟权衡，以及 ffmpeg 兼容路径。
sidebar:
  order: 5
---

媒体引擎负责把 DASH 上游变成播放器可以直接消费的 HLS。原生引擎在 Kiln 进程内完成解密与重封装，不需要外部进程；ffmpeg 只作为兼容回退存在。
...
| 取值 | 行为 |
| --- | --- |
| `auto` | 源能被原生引擎处理时走原生；不能处理且允许回退时改用 ffmpeg |
...
`auto` 会在原生规划失败时评估这次失败能否安全交给 ffmpeg，答案是否定时宁可报错也不换引擎：

- 一个源里出现多个 KID 时禁止回退。ffmpeg 路径只接受单个 CENC 密钥，交给它会解出错误的画面，此时返回 `502`。
- 频道显式选择了字幕轨（`subtitles.mode` 为 `prefer` 或 `only`）时，兼容引擎无法履行这个选择，请求被拒为 `400`。
- 需要回退但运行环境里没有 ffmpeg 时返回 `503`。Core 与 Lite 镜像不含 ffmpeg，详见 [Core、Full 与 Lite](/guide/variants/)。

`/readyz` 会把这条边界前移：只要存在 `engine` 为 `ffmpeg` 的 DASH 频道而 ffmpeg 不可用，就返回 `503`，避免编排系统把流量导到一个注定失败的实例上。

## 原生引擎

原生引擎读取 MPD，规划轨道，逐段下载、解密并重封装成 CMAF 分片，最后写出 HLS 播放列表。整条链路都在 Go 里完成，不落 ffmpeg，也不依赖任何外部二进制。
...
<Aside type="tip" title="资源自适应会下压这些值">
  `server.resource_mode` 为 `auto` 或 `constrained` 时，运行时会按实际可用内存与 CPU 收紧 `inflight_bytes`、`max_segment_bytes`、`start_segments` 与 `prefetch_segments`，只下压、不上调。生效值会打印在启动日志里，参见 [运维](/guide/operations/)。
</Aside>
...
| `hls_list_size` | `8` | 输出播放列表保留的分片数 |
| `prefer_height` | 未设 | 目标分辨率上限，频道级 `prefer_height` 优先 |
| `low_latency` | `false` | 只在 `hls_list_size` 未给出时决定其默认值，实际部署里通常由 `hls_list_size` 直接决定 |
| `max_starts` | `1` | 并发启动 ffmpeg 的上限。它不覆盖就绪等待，所以一个慢源不会阻塞其它频道的冷启动 |
...
<Aside type="caution" title="ffmpeg 的内存不在 Go 预算内">
  资源自适应给出的是 Go 堆与媒体工作集的软预算。ffmpeg 是独立进程或独立容器，它的内存完全在预算之外。运行在受限档位、同时又可能启动 ffmpeg 时，启动日志会给出一条 advisory 警告。
</Aside>
