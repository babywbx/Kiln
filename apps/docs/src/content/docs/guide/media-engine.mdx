...
直播会话启动时先为全部轨道各发布一个起始窗口，此后回源跟着播放需求走：主视频轨作为整条流水线的时钟始终拉取，其余轨道在 `rendition_idle_sec`（默认 30 秒）内没有任何播放请求就暂停回源，播放器再次请求时自动跳回直播最前沿，并在播放列表里以 `EXT-X-DISCONTINUITY` 标注这次跳跃。单观众场景下这能把入方向带宽压到只剩正在观看的轨道；把该键设为负数即可恢复全量拉取。点播不受影响。
