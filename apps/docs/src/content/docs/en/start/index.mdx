---
title: Quick start
description: Three deployment paths and the shortest path to a verified install.
sidebar:
  order: 1
---
...
<CardGrid>
  <Card title="Install script" icon="ph:terminal">
    One command on Linux and macOS; run it again to upgrade, or register a systemd unit. See [Install script](/en/start/install-script/).
  </Card>
  <Card title="Docker" icon="ph:cube">
    Three image variants (core, full, lite) with a Compose template, health checks, and container resource adaptation. See [Docker](/en/start/docker/).
  </Card>
  <Card title="Manual binary" icon="ph:hard-drives">
    Download an archive from Releases, verify it against `SHA256SUMS`, and place it yourself. Windows runs it as a service. See [Binary](/en/start/binary/).
  </Card>
</CardGrid>
...
`hls-demo` is the channel id from the example config; swap in your own. `/v1/playlist.m3u` accepts a login session only, while everything under `/v1/play/` takes either a `?token=` query parameter or an `Authorization` header. The full credential split is documented in the [API reference](/en/reference/api/).
