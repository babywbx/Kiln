...
Kiln ships three image variants with explicit boundaries, sharing one configuration model and the same native media modules. Which one you want comes down to two questions: do you need the FFmpeg compatibility fallback, and do you need the database and admin console.

## The three variants

| Image | Capabilities | Default packager | FFmpeg | Purpose |
| --- | --- | --- | --- | --- |
| `lite` | Config, login, M3U, playback | `native` | Not included | Minimal scratch runtime, 3.8 MB, no database |
| `core` | Full | `native` | Not included | Complete management and observability, natively only |
| `full` | Full | `auto` | Bundled | Native first, falls back when needed |
...
| Tag shape | Meaning |
| --- | --- |
| `:1.0.0`, `:1.0`, `:1`, `:latest` | Exact, minor, major, and newest release of the full variant |
| `:1.0.0-core`, `:1.0-core`, `:1-core`, `:core` | The same set for core |
| `:1.0.0-lite`, `:1.0-lite`, `:1-lite`, `:lite` | The same set for lite |

A prerelease publishes only its exact version tag and never moves `latest`, `core`, `lite`, or the major and minor tags. Core builds for `linux/amd64`, `linux/arm64`, `linux/arm/v7`, and `linux/arm/v6`; full and lite build for `linux/amd64` and `linux/arm64`. All three get build provenance attestations pushed to the registry at release time.
...
<Aside type="caution" title="Do not pin the ffmpeg engine on a native-only image">
  Core and lite ship without ffmpeg. If a DASH channel resolves to the `ffmpeg` engine, `/readyz` returns 503 stating that the compatibility engine is unavailable, and the container never reaches a ready state. Switch the channel to `native`, or use the full image.
</Aside>
