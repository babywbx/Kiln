...
`-healthcheck` performs normal certificate validation. A manual HTTPS probe fails until an auto-generated certificate is in the system trust store; with split listeners, prefer the plaintext health port. See [HTTPS and split listeners](/en/guide/https/).
