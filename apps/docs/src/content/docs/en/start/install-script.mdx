...
One command installs Kiln on Linux and macOS; run it again to upgrade. The script does four things: detect the platform, pick a working download source, verify `SHA256SUMS`, and swap the binary atomically. No sudo by default, and the full plan is printed before anything is written.
...
If the download stalls or fails, prefix a mirror:
...
| `--version <v>` | Pin a version, with or without the leading `v`; must be valid semver and may carry a prerelease suffix | Latest release |
...
Every option has an environment variable; command-line flags win over them.
...
Three GitHub proxies are built in: `ghfast.top`, `ghproxy.net`, and `gh-proxy.com`.
...
<Aside type="tip" title="Mirrors only carry bytes">
...
Re-running the script is the upgrade path. Comparison follows full semver rules, including per-identifier prerelease ordering, so `1.0.0-rc.1` correctly sorts before `1.0.0`.
...
On a constrained network, swap the first line for a mirror URL and leave the second unchanged.
...
```bash
sudo systemctl disable --now kiln && sudo rm /etc/systemd/system/kiln.service
```
...
After the download, the script fetches `SHA256SUMS` (direct with a 5 second timeout, falling back to the active source with a 10 second one) and compares the line matching this artifact. A mismatch discards the downloaded file and exits 4 without touching the existing install.
