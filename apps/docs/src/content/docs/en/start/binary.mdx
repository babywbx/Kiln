...
<Steps>
  <Step title="Download the archive and the checksums">
    Get the archive for your platform from the [Releases](https://github.com/babywbx/Kiln/releases) page, together with `SHA256SUMS`, into the same directory.
  </Step>
...
To keep it running across reboots on Linux, the install script's [`--service`](/en/start/install-script/) flag registers a systemd unit. It enables the unit immediately when the config exists; otherwise create the config and run `systemctl enable --now kiln`.
