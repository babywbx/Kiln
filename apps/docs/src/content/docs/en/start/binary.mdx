...
When the [install script](/en/start/install-script/) is not an option, or the target is Windows, grab a release archive from GitHub. Every archive is one static binary plus the license and readme files: unpack and run, no runtime dependencies.
...
<CardGrid>
  ...
  <Card title="An easier install">
    On Linux and macOS, one command handles installs and upgrades: [Install script](/en/start/install-script/).
  </Card>
  <Card title="It will not start">
    Startup failures, port conflicts, and permission problems are covered in [Troubleshooting](/en/guide/troubleshooting/).
  </Card>
</CardGrid>
