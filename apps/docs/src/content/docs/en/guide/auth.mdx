...
Kiln answers "who is calling" with four credentials that never overlap. Public endpoints need none. Login sessions serve the admin console. Administrator API tokens serve scripts and automation. Path-based playback keys serve players and playlist distribution. None of them can promote itself into another.
...
| Administrator API token | `kiln_v1_` prefix plus 48 random characters | Scripts, CI, external tooling | Expiry set at creation; rotatable and revocable |
| Playback key | `v1` prefix plus 126 random characters, carried in the URL path | Players and playlist distribution | Expiry set at creation; revocable |
...
`POST /v1/auth/login` exchanges a username and password for an Ed25519-signed JWT. The response also carries the expiry, the username, and the role.
...
- Issued tokens carry `iss`, `aud`, `exp`, `iat`, `nbf`, `sub`, and a single-use `jti`. All time-based assertions are enforced on verification, with 30 seconds of clock skew allowed.
...
- Expiry is set at creation and can be adjusted later, up to ten years. Leave it unset for a token that never expires.
- **Rotation** keeps the name, scopes, and expiry and replaces only the secret. The old value stops working immediately and the new plaintext is again shown once. Prefer rotation when replacing a leaked credential.
...
| `expired` | 401 | The token has passed its expiry |
...
| `allowed_hosts` | Outbound allowlist of origin hosts Kiln may fetch from. Hosts already declared by upstreams and channel source URLs are merged in automatically, so this usually only needs the extra domains that redirects land on |
...
On top of that, every response carries `X-Content-Type-Options`, `Referrer-Policy`, `X-Frame-Options: DENY`, a strict `Content-Security-Policy`, and `Cache-Control: no-store`, which also means the console cannot be embedded in a third-party page.

## Playback keys

A playback key is a separate credential meant for players and playlist distribution. It travels in the URL path (`/p/{token}/...`), can be scoped to a subset of channels with an expiry, can be revoked at any time, and every use is written to the playback access log with only the key prefix retained in the recorded path. It is fully isolated from login credentials, so handing a playback URL to a third-party device leaks no administrative access.
...
<Steps>
  ...
  <Step title="Tighten the host allowlists">
    Use `public_hosts` to pin the domains served and `allowed_hosts` to pin the origins fetched. Leaving both empty means any `Host` reaches the service and the outbound surface is decided entirely by channel configuration.
  </Step>
  <Step title="Give automation its own token">
    Scripts should always use an administrator API token with the minimum scopes for the job, a real expiry, periodic rotation, and the audit log as the record of who called what.
  </Step>
</Steps>
