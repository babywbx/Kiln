...
description: The channel model field by field, upstream definitions, config versus console ownership, and enable, import, and self-check operations.
sidebar:
  order: 2
---

A channel is the only real object in Kiln. It binds one upstream address, one ingress format, and one run policy together, and shows up externally as a single playback URL plus a line in the playlist. This page covers that model field by field.

## The channel model

### Identity and presentation

- `id`: the unique identifier. It appears in playback URLs, in the playlist's `tvg-id` attribute, and in access logs. Changing it creates a different channel as far as players are concerned, so favourites and history on the client side are lost.
...
- `idle_timeout_sec`: how long counts as nobody watching. Values of zero or below normalise to `90` seconds. Only applies to `on_demand` channels.
...
| `autostart` | `on_demand` | Console label | Behaviour |
| --- | --- | --- | --- |
...
<Aside type="note" title="Autostart at boot">
  Startup autostart follows the channels declared in the configuration file. Channels created only in the admin console should be brought up with Start now or by their first viewer.
</Aside>
...
- `selection`: finer-grained track selection with a separate mode and track for video, audio, and subtitles. See [Variants & tracks](/en/guide/variants/).
- `packager`: a per-channel engine override taking the same values as `[packager].engine`. An invalid value is ignored and the global setting applies.
...
- `proxy`: pin this upstream to a specific outbound route. Leave it empty to fall back to the rules in [Outbound proxying](/en/guide/proxy/).
...
The two are not the same store, and the boundary is worth memorising:
...
- Disabling only hides the channel. It deletes nothing and does not disturb the channel's position in the list.
- A running session is not torn down by disabling. Stop it explicitly with `DELETE /v1/admin/sessions/{id}` when you need to.
