...
### Repairing an incorrectly downgraded redirect

Some HTTPS origins incorrectly redirect to an `http://` URL on a public host's default port. Enable `upgrade_insecure_redirects = true` on a shared upstream only for that case. A channel with a direct `source_url` needs the same option on the channel itself. It upgrades redirect targets only; it does not turn an initial HTTP source into HTTPS and never changes private, `.local`, `.internal`, non-default-port, or explicitly `security.allowed_hosts` targets.

Channels inherit an enabled shared-upstream setting and may opt in themselves, but a channel-level `false` cannot cancel an enabled upstream. The option is off by default and is not a general workaround for unreachable origins or certificate failures.
...
- **Outbound routes**: `[[proxies]]` and `[[egress.rules]]` are synchronized from the config file by ID on every start; console-only IDs remain. The three `egress.*` scalar settings are synchronized as well, and omitted keys still write the defaults `direct`, `rewrite`, and `host.docker.internal`.
- **EPG sources**: seeded on first start only, database-authoritative afterwards.
