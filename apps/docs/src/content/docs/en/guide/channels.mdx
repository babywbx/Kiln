...
- **Outbound routes**: `[[proxies]]`, `[[egress.rules]]`, and the `egress.*` keys are synchronized from the config file on every start. Anything the file does not declare keeps the value set from the console.
