---
title: Core, Full & Lite
description: Capability boundaries of the three distribution variants, how they are built and shipped, Lite's fixed resource budget, controlled benchmark results, and how to pick one.
sidebar:
  order: 6
---

Kiln ships three variants. They share one configuration model and the same native media modules; what differs is how complete the control plane is, whether ffmpeg is bundled, and how tight the resource budget is.

## The three variants

| Dimension | Core | Full | Lite |
| --- | --- | --- | --- |
| Default packager | `native` | `auto` | `native` |
| Native HLS / DASH | Yes | Yes | Yes |
| FFmpeg | Not included | Bundled, 8.1.2 | Not included |
| Login, M3U, playback | Yes | Yes | Yes |
| SQLite database | Yes | Yes | No |
| Admin console and admin API | Yes | Yes | No |
| EPG | Yes | Yes | No |
| OTLP and pprof | Yes | Yes | No |
| Base image | Alpine | Alpine plus FFmpeg | `scratch` |
| Purpose | Complete, purely native deployment | Complete deployment with compatibility | Fixed-config, low-resource playback node |
...
The default packager an image provides is injected through `KILN_DEFAULT_PACKAGER_ENGINE` and only applies when the config omits `[packager].engine`. The same config never changes behaviour just because the image tag changed — see [Media engine](/en/guide/media-engine/).
