...
- In the full build, `[epg].enabled` is a statement of intent: what actually decides whether a guide is served is whether any source is enabled. The lite build has no EPG at all, and refuses to start when `enabled = true`.
