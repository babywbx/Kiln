...
Fetched XMLTV is parsed as it streams in and written straight into a SQLite database, so the guide never lives in memory as a whole and is read back per channel and time window on demand. `cache` and `cache_dir` together decide where that database lives.
