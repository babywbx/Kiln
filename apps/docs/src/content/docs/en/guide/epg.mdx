...
Fetched XMLTV is parsed as it streams in and written straight into a SQLite database, so the guide never lives in memory as a whole and is read back on demand for each matched channel. `cache` and `cache_dir` together decide where that database lives.

<Tabs>
  <TabItem label="Disk (default)">
    `cache = true` with `cache_dir` pointing at a directory, `{data_dir}/epg` by default, holding `epg.db`.

    The database survives restarts, feeds `If-None-Match` and `If-Modified-Since` on the next fetch so an unchanged source costs a 304 and skips parsing entirely, and keeps serving the previously ingested guide when a fetch or parse fails, flagging that source as stale. Disk usage is roughly the decompressed size of the sources.

    Raw XMLTV files named `<sha256>.cache` from older versions are not imported into `epg.db`. The first start after upgrading performs a full fetch, and the affected guide may stay empty until it succeeds. Keep the old files if rollback matters, and remove them only after the new database has refreshed successfully.
  </TabItem>
  <TabItem label="Memory">
    `cache = true` with `cache_dir = "memory"` (`:memory:` is equivalent).

    Conditional requests and failure fallback behave exactly as on disk, but nothing is written to the filesystem and every source is downloaded in full again after a restart. Use it on read-only root filesystems, or wherever you do not want large files accumulating in the data directory. Memory use still scales with the source size, but Kiln no longer retains a second parsed object graph.
  </TabItem>
  <TabItem label="Off">
    `cache = false`.

    Nothing is written to disk and a process-local database is used instead, so there are no conditional requests or fallback across restarts. On top of that, every request to `/v1/epg.xml` or `/v1/epg.xml.gz` synchronously refreshes all sources before rendering the response.
  </TabItem>
</Tabs>
...
- An oversized download fails the round. Existing data stays available and is flagged as stale rather than being replaced by a truncated body. The limit trips while streaming, so an oversized body is never buffered in full.
- Under `auto` the limit is lowered with memory: below 256 MiB of memory it drops to 4 MiB, and other constrained profiles land somewhere between 4 MiB and 64 MiB depending on available memory. In disk mode it primarily bounds download and database size rather than a retained Go object graph.
