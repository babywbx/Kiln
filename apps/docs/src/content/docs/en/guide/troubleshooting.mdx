...
**How to tell**: HTTP, HTTPS, and `socks5h` profiles hand the original hostname to the proxy for resolution, while `socks5` resolves on the Kiln host first. In every case Kiln still performs local DNS and SSRF validation before the proxy connection. Errors such as `probe target dns lookup failed` or `resolved to a non-public address` can therefore occur before the proxy takes over.

**Fix**: prefer DNS that returns real addresses to Kiln, or use a trusted HTTP(S) or `socks5h` profile. Add only exact, genuinely trusted private origins to `security.allowed_hosts`; never broadly disable private-network validation.
