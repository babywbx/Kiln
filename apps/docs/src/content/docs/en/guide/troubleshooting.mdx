...
<Aside type="tip" title="Start with two log lines">
  Most problems are answered by the startup logs. `listening` reports the address and `scheme` Kiln is about to serve; split mode emits one HTTP and one HTTPS record, though a later bind can still fail. The `kiln starting` record prints the version, config path, channel count, resource profile, `packager_engine`, `ffmpeg_available`, proxy count, and database path. Confirm those records are present before digging further.
</Aside>
...
First confirm which credential you are using. Four credential types cover four surfaces, and mixing them up is the most common cause: public endpoints need none, session and preview JWTs serve the admin UI and protected playback, admin API tokens serve scripts, and path-style play keys serve player distribution. See [Authentication](/en/guide/auth/).
...
**Fix**: with `play_require_auth` enabled, `/v1/playlist.m3u` and `/v1/play/...` accept a session or preview JWT through Bearer or `?token=`, but never an admin API token. `/p/<token>/...` accepts only the playback key in its path. With playback auth off, the first two surfaces are public and generated URLs carry no token. API tokens are for management endpoints, some of which remain session-only.

### play_require_auth

**Symptom**: you disabled playback auth but `/v1/playlist.m3u` or `/v1/play/...` still demands a credential.
...
<Aside type="danger" title="Never disable playback auth in production">
  With it off, anyone can fetch the complete playlist and pull every stream. This is a debugging switch, not a deployment option.
</Aside>
...
If the log shows an HTTPS source redirecting to `http://...:80` on a public host, enable `upgrade_insecure_redirects` for that upstream or direct-URL channel as described under [Channels](/en/guide/channels/#repairing-an-incorrectly-downgraded-redirect). It only repairs that malformed redirect, not ordinary network, DNS, or TLS certificate failures.
...
First distinguish the three timeouts by the stage that failed:

| Stage | Default | Meaning |
| --- | ---: | --- |
| Waiting for response headers | Fixed and not configurable | The request connected but the origin never returned headers |
| `packager.fetch_stall_sec` | 30 seconds | No new body byte arrived for the whole window, covering manifests, segments, and error bodies; any progress resets it, so it is not a total download limit |
| `packager.stall_timeout_sec` | 180 seconds | The manifest keeps refreshing but no track publishes a segment; `-1` disables it |

A large, slow segment may take longer than 30 seconds as long as bytes keep arriving. A truly frozen transfer hits `fetch_stall_sec`; a progressing manifest with no new media hits the publication-stall detector below.
...
**Fix**: the disk database defaults to `data_dir/epg/epg.db`. Fix the network, proxy, or size limit first and trigger a refresh. Only for confirmed database corruption, stop Kiln, move the `epg` directory aside as a backup, and restart. Legacy `<sha256>.cache` files are not imported. The refresh interval comes from `epg.refresh_interval_min`, 360 minutes by default; lowering it means watching out for upstream rate limits. See [EPG](/en/guide/epg/).
...
### DNS, TUN, or fake-IP environments

**How to tell**: every profile resolves the target on the Kiln host, applies SSRF validation, and gives the pinned IP to the direct connection or proxy; `socks5h` no longer triggers a second proxy-side lookup. Errors such as `probe target dns lookup failed` or `resolved to a non-public address` therefore occur before the proxy takes over. A plain HTTP upstream through an HTTP(S) forward proxy is rejected because that path cannot both pin the dial address and preserve the original Host.

**Fix**: give Kiln DNS that returns real, reachable addresses. On a host where TUN or fake-IP applies system-wide and no process can obtain a real address, set `egress.trust_proxy_dns = true` to hand resolution back to the proxy, at the cost of Kiln no longer validating the destination IP; see [Egress proxy](/en/guide/proxy/). Move a plain HTTP upstream to HTTPS or use SOCKS5. Add only exact, genuinely trusted private origins to `security.allowed_hosts`; never broadly disable private-network validation.
...
**How to tell**: the `core` and `full` health checks use `wget --no-check-certificate` to try HTTPS first and then HTTP. Scratch-based `lite` uses the binary subcommand against HTTP. Reproduce the check for the image you run:
...
A manual `-healthcheck` against auto-generated HTTPS fails default certificate validation. Prefer the plaintext port in split mode; for HTTPS-only mode configure a certificate issued for the deployment hostname or use the image probe above. See [HTTPS and split listeners](/en/guide/https/).
