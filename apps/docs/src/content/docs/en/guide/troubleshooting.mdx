...
A manual `-healthcheck` against auto-generated HTTPS fails until the certificate enters the system trust store. Prefer the plaintext port in split mode; for HTTPS-only mode use a trusted certificate or the image probe above. See [HTTPS and split listeners](/en/guide/https/).
