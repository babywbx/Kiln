...
**Symptom**: saving a channel's proxy binding is rejected with `FFmpeg cannot use a SOCKS proxy; choose HTTP, direct, or the native DASH engine`, or a session fails with `CONNECT tunnel cannot use socks5 proxy`.

**How to tell**: every compatibility-engine network request first passes through Kiln's local guarded proxy and then enters the channel's outbound route. That handoff supports `http` and `https` profiles. The native engine supports all four (`http`, `https`, `socks5`, `socks5h`).
