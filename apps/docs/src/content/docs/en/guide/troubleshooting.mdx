...
**Fix**: give Kiln DNS that returns real, reachable addresses; do not depend on a second proxy lookup in TUN or fake-IP environments. Move a plain HTTP upstream to HTTPS or use SOCKS5. Add only exact, genuinely trusted private origins to `security.allowed_hosts`; never broadly disable private-network validation.
