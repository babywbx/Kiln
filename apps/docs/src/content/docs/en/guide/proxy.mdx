...
<Aside type="caution" title="Local DNS is a security prerequisite">
  Kiln must obtain the target's real, reachable IP in its own network before it can validate SSRF policy and bind the actual dial. If TUN or fake-IP DNS returns a synthetic address that has no meaning in the proxy network, configure real DNS resolution for upstream hosts. Kiln does not fall back to unchecked proxy-side resolution.
</Aside>
