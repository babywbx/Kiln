...
Kiln uses two layers to manage outbound traffic: `[[proxies]]` defines reusable proxies, and `[egress]` decides which requests use each route. Stream sources, guide sources, and logo requests share the global routing table, while channels and guide sources can override it.
...
<Steps>
  ...
  <Step title="Resolve the route">
    `direct` connects directly, and a proxy ID uses that route. A startup config that references a missing or disabled route prevents startup; an invalid admin update is rejected while the current runtime configuration stays active.
  </Step>
</Steps>

Resource-level settings take precedence over global rules:

| Resource | `auto` | `direct` | Named route |
| --- | --- | --- | --- |
| Channel | Global rules match the `channel_id` and URL | Always direct | Always use that route |
| EPG source | Global rules match the URL | Always direct (default) | Always use that route |
| Logo | Inherits its channel's egress context | Inherits the channel | Inherits the channel |

The channel editor's **Egress route** and an EPG source's `proxy` setting both offer `auto`, `direct`, and named routes. Only `auto` continues into the global rules below.
...
<Aside type="caution" title="The config file is synchronized on every start">
  `[[proxies]]` and `[[egress.rules]]` are written to the database by ID on every start, so console edits to the same IDs are overwritten; routes and rules created only in the console remain when the file has no matching ID. `default`, `playlist_policy`, and `docker_proxy_host` are also synchronized from the loaded config on every start. Omitting them still supplies the defaults `direct`, `rewrite`, and `host.docker.internal`. Put the final global egress values in the config file if console changes must survive a restart.
</Aside>
...
- Every route first resolves and validates the target in Kiln, then gives the pinned IP to the direct connection or proxy while retaining the original HTTP Host and target TLS SNI. `socks5h` remains accepted as a compatibility scheme, but its target also uses Kiln's pinned IP rather than a second proxy-side lookup.
- For an HTTPS upstream through an HTTP or HTTPS forward proxy, CONNECT names the validated pinned IP. Go's standard forward-proxy path cannot express both a pinned dial address and the original Host for a plain HTTP upstream, so that combination is rejected before connecting; use an HTTPS upstream or a SOCKS5 route instead.
...
<Aside type="caution" title="Local DNS is a security prerequisite">
  Kiln must obtain the target's real, reachable IP in its own network before it can validate SSRF policy and bind the actual dial. If TUN or fake-IP DNS returns a synthetic address that has no meaning in the proxy network, configure real DNS resolution for upstream hosts.
</Aside>

That prerequisite does not hold on a host where TUN or fake-IP DNS is applied system-wide and no process can obtain a real address. Such a deployment can set `egress.trust_proxy_dns = true` to hand the hostname to the proxy, which restores the behavior from before v1.1.0:
...
<Aside type="caution" title="The proxy becomes the trust boundary">
  Kiln no longer learns the real destination IP, so it cannot check whether the target is private, a metadata endpoint, or otherwise blocked. Only the scheme and hostname checks remain (`localhost`, `.local`, `.internal`, and metadata hosts are still refused). Do not enable it when upstream URLs are not fully under your control, and block private destinations on the proxy. The switch applies only to proxied requests; direct requests still resolve and pin locally.
</Aside>

<Aside type="note" title="Both engines support all four schemes">
...
To proxy all origin fetches by default, define a `[[proxies]]` entry and set `egress.default` to its ID. Kiln does not use `HTTP_PROXY`, `HTTPS_PROXY`, or `ALL_PROXY` as egress configuration.
...
| `auto` | URLs that need a proxy are rewritten; when the channel has a custom User-Agent or effective upstream headers, every URL is rewritten back to Kiln |
...
`passthrough` sends the player straight to the origin. Those requests bypass Kiln's proxy routes and never carry the channel's custom headers, so it only makes sense when the player itself can reach the origin. `auto` normally rewrites only URLs that need a proxy. If the channel has a custom User-Agent or any effective upstream header, every URL is rewritten instead, because a player connecting directly would lose that configuration.

Kiln's outbound host checks apply only to targets rewritten under `/u/` and fetched by Kiln. Origin URLs retained by `passthrough` or `auto` are opened directly by the player and do not pass through Kiln's SSRF validation. That is an inherent boundary of direct delivery; use `rewrite` for an untrusted upstream playlist.
...
<Aside type="caution" title="The host proxy must accept LAN connections">
...
<CardGrid>
  ...
  <Card title="Kiln in a container, proxy on the host" icon="ph:cube">
    Point the route at `http://host.docker.internal:7890` and give the Kiln container a `host.docker.internal:host-gateway` mapping. See [Reaching a proxy from inside a container](#reaching-a-proxy-from-inside-a-container).
  </Card>
  <Card title="Kiln and the proxy in separate containers" icon="ph:graph">
    Put both on one Docker network and point the route at `http://<service>:<internal port>`; the proxy publishes nothing. See [Reaching a proxy from inside a container](#reaching-a-proxy-from-inside-a-container).
  </Card>
</CardGrid>
