...
- HTTP, HTTPS, and `socks5h` routes preserve the original hostname for proxy-side resolution. This is the right choice when Kiln runs behind TUN or fake-IP DNS and only the proxy can resolve the real destination; `socks5` still uses Kiln's local resolution.
...
<Aside type="caution" title="Proxy-side DNS is a trust boundary">
  Kiln rejects destinations that resolve locally to private or special addresses before proxying. HTTP, HTTPS, and `socks5h` proxies resolve the hostname again, and Kiln cannot inspect that result. Use trusted proxies and enforce private-destination blocking on the proxy when fetched URLs are not fully controlled.
</Aside>
