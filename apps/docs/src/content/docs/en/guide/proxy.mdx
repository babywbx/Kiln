...
description: Define reusable proxy routes and host-based rules that decide how Kiln reaches origins, guide sources and logo files.
sidebar:
  order: 8
---

Outbound routing has two layers: `[[proxies]]` defines reusable routes, and `[egress]` decides which traffic takes which route. Channels, EPG sources and logo fetches all share the same routing table, so there is no per-feature proxy setting to keep in sync.
...
- `id` must be unique and is what `egress.default`, rules and EPG sources reference. `name` is display only.
...
A rule consists of `id`, `priority`, `kind`, `pattern` and `proxy`, and can be disabled individually. Lower `priority` numbers win. `proxy` may also be `direct`, which is how you punch a direct hole through a deployment that otherwise proxies everything.
...
Under `rewrite`, Kiln resolves every media line and every `URI="..."` attribute to an absolute URL, then replaces it with `/v1/play/<channel id>/u/<encoded url>?sig=...` on its own origin. Behind an access token the prefix becomes `/p/<token>/play/<channel id>/u/` instead. The URL is encoded with URL-safe Base64 and keeps a recognisable extension, and the signature is an HMAC over the channel id and target URL, keyed by a random value generated at process start, so rewritten URLs stop working after a restart. The player therefore only ever contacts Kiln, and Kiln performs the upstream fetch through whichever route the rules selected. Nested variant playlists are rewritten again as they are pulled.
...
| EPG fetches | Routed by the rules only when the source's egress is `auto`; `direct` or a named route is honoured as configured, see [EPG](/en/guide/epg/) |
