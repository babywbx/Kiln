...
**Active Sessions** lists the running playback sessions with channel, state, type, engine, packaging mode, and last error. Select an error summary to view and copy the complete message in a dialog, or stop a session from the action at the right. Stopping is immediate: players currently watching are interrupted, and the session restarts on the next playback request.
...
The switch in each row drives the session directly: an idle channel shows ▷ and starts warming up when clicked; a channel with a session (running, starting, restarting, or failed) shows ■ and stops it when clicked, so a stalled cold start can always be cut short. It only affects the current session and is separate from enabling or disabling the channel.
...
Expand **Advanced request settings** to configure the user agent, extra request headers, insecure redirect upgrades, and failure-recovery behavior. Sensitive headers are not shown again after saving. **Network egress** lets a channel follow the global rules, use a direct connection, or use a specific proxy. You can also paste a new proxy URL to create one from this page.
...
The settings page holds five cards, top to bottom.
...
**HTTPS** toggles built-in TLS, which takes effect after a restart. The card reports whether listening is single or split, the TLS port, certificate source and path, covered hosts, expiry, or a loading error. A self-signed certificate also gets a trust-store reminder. With one listener, the card checks the public base URL scheme. A split listener allows either port, so verify both the scheme and port against the player's actual entry point. See [HTTPS and split listeners](/en/guide/https/) for deployment details.
