---
title: Operations
description: Running Kiln for the long haul, covering systemd and Windows services, the data directory and backups, upgrades, logging, health checks, metrics, OTLP tracing, pprof, and resource adaptation.
sidebar:
  order: 10
---

This page covers everything that happens after Kiln is up: wiring it into the init system, knowing what lives in the data directory and how to back it up, upgrading safely, and configuring logs, health checks, metrics, traces, and the resource adaptation layer.
...
The simplest backup is to stop the process and copy the whole directory. For an online copy, always include `kiln.db-wal`, because grabbing only the main file loses everything not yet checkpointed. You can skip `sessions/` entirely; it is wiped and rebuilt whenever a session starts.
...
The binary ships a health-check subcommand with a 3 second timeout. Exit code 0 on 2xx, 1 otherwise:
...
The endpoint is gated on `[observe].enabled`, which config loading turns on, so `core` and `full` always serve `/metrics`. `lite` does not register the route at all.
...
The exporter is only initialized when `[observe].otlp_endpoint` is set. Leave it empty and tracing costs nothing.
...
`effective_memory_mb` is what was detected, `memory_limit_mb` is the Go soft target that was written, and `effective_go_memory_limit_mb` is what the runtime actually applies. When the three disagree, check for `GOMEMLIMIT` first. A `resource_profile` of `configured` means memory detection returned nothing and profile selection was skipped entirely. Set `KILN_RESOURCE_MEMORY_MB` explicitly in that case.
...
<CardGrid>
  <Card title="Troubleshooting" icon="ph:first-aid-kit">
    Symptom tables, real log keywords, and fixes in [Troubleshooting](/en/guide/troubleshooting/).
  </Card>
  <Card title="Configuration reference" icon="ph:sliders">
    Every config section, field, and default in the [config reference](/en/reference/config/).
  </Card>
  <Card title="Environment variables" icon="ph:terminal-window">
    All `KILN_` variables and their precedence in the [environment reference](/en/reference/env/).
  </Card>
</CardGrid>
