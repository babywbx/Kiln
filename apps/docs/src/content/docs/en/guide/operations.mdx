...
Passing `--service` to the install script registers a systemd unit. It is enabled and started only when `/etc/kiln/kiln.toml` already exists. This step needs root and only works on Linux with systemd available.
...
| `epg/` | EPG guide database `epg.db`, used when `epg.cache_dir` is empty | No, refetchable |
...
<Aside type="note" title="Legacy EPG cache files are not migrated">
  Older `<sha256>.cache` files under `epg.cache_dir` are not imported into the new `epg.db`. The first start after upgrading refetches every enabled guide source, so the guide may stay empty until that succeeds. Keep the old files if rollback matters and remove them only after `epg.db` has refreshed successfully.
</Aside>
...
<CardGrid>
  <Card title="/healthz" icon="ph:heartbeat">
    Liveness. Returns `200` and `{"status":"ok"}` as long as the service listener is running; it checks no dependencies. Use it for process supervision and container restart policies.
  </Card>
...
The subcommand uses the system trust store and performs normal certificate validation; it has no insecure or custom-CA flag. An HTTPS probe fails until an auto-generated certificate is trusted, so split mode should probe the plaintext health port. The images already declare `HEALTHCHECK`: `core` and `full` use `wget --no-check-certificate` to try HTTPS before HTTP, while scratch-based `lite` uses the subcommand above against HTTP. See [HTTPS and split listeners](/en/guide/https/) for listener and certificate details.
