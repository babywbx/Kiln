---
title: Media engine
description: The three engine modes, what the native DASH decrypt-and-repackage path supports, key files, LL-HLS parameters, the memory-versus-latency trade-off, and the ffmpeg compatibility path.
sidebar:
  order: 5
---

The media engine turns a DASH upstream into HLS that players can consume directly. The native engine decrypts and repackages inside the Kiln process, with no external binary involved; ffmpeg exists only as a compatibility fallback.
...
- If a channel selects a subtitle track explicitly (`subtitles.mode` set to `prefer` or `only`), the compatibility engine cannot honour that selection and the request is rejected with `400`.
- If a fallback is needed but no ffmpeg is available in the runtime, the request fails with `503`. The Core and Lite images ship without ffmpeg — see [Core, Full & Lite](/en/guide/variants/).
...
The native engine reads the MPD, plans the tracks, then downloads, decrypts, and repackages segment by segment into CMAF before publishing HLS playlists. The whole chain runs in Go, with no ffmpeg and no external dependency.
...
<Steps>
  <Step title="Plan">
    Fetch and normalise the MPD, then derive the ABR ladder, the audio renditions, and the subtitle tracks from `prefer_height` and the channel's track selection.
  </Step>
  <Step title="Verify">
...
| `multi_kid_cannot_fall_back` | The source uses several KIDs, which single-key ffmpeg would decode wrongly | No |
...
| `hls_list_size` | `8` | Segments kept in the output playlist |
| `prefer_height` | unset | Resolution ceiling; a channel's `prefer_height` wins |
| `low_latency` | `false` | Only picks a default for `hls_list_size` when that is absent, so in practice `hls_list_size` decides directly |
