...
| `playlist_policy` | string | `"rewrite"` | Playlist URL rewriting policy; one of `rewrite`, `passthrough`, `auto`. Anything else fails validation. `rewrite` always rewrites to a Kiln-relative URL and `passthrough` always keeps the original. `auto` rewrites URLs that use a proxy; it also rewrites every URL when the channel has a custom User-Agent or effective upstream headers so direct player requests do not lose that configuration. |
...
| `min_test_kbps` | int | `1000` | Throughput floor for the connection test. It reads up to 4 MiB or 5 seconds and measures the bytes delivered when the window expires; below this rate the result is reachable but too slow for playback. A response that ends before 256 KiB is not treated as a bandwidth sample and reports reachability only. A negative value disables the verdict. |
| `trust_proxy_dns` | bool | `false` | Let the proxy resolve the destination instead of resolving and pinning it locally. Only for hosts running TUN or fake-IP DNS, where no process can obtain a real address. Kiln can no longer validate the destination IP and keeps only the scheme and hostname checks, so point it at a trusted proxy only. Configuration file only; the console cannot change it. |
