...
`[[channels]]` seeds the SQLite database under `server.data_dir` only while the channel table is empty. After that the admin surface owns them, and editing the config file no longer overwrites them. `[[proxies]]`, `[[egress.rules]]`, `egress.default`, `egress.playlist_policy`, and `egress.docker_proxy_host` are instead **synchronized from the config file on every start**: whatever the file declares wins, whatever it omits keeps the value set from the console, and a write only happens (advancing the revision) when the file genuinely differs. When the file carries no credentials but the stored proxy has them, the credentials are kept. Proxies and rules created from the console that the file never mentions are left in place. `server.public_base_url` still uses insert-if-absent semantics and only applies on first run. `[[upstreams]]`, `[auth]`, and the remaining global keys are always taken from the file, though the username and password of an entry in `[[auth.users]]` can be changed from the admin surface and stored as a database override.
...
| `tls_listen` | string | `""` | Optional. When HTTPS is enabled, setting this adds a full-featured TLS listener at this address while `listen` stays plaintext and is limited to the public playback and probe surface. Must use a numeric port and differ from `listen`. Leave empty to apply TLS to the single listener. |
...
TLS is available in Core and Full. Lite rejects `tls_enabled = true`, a `tls_listen` address, or either certificate path instead of silently serving HTTP.
