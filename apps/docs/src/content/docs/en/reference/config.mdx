...
This page lists Kiln's configuration format section by section. The default column shows the value the code actually falls back to, which is not always the value the example files write out — the examples are a working starting point, not a list of defaults.
...
<Steps>
  ...
  <Step title="Apply environment overrides">
    Apply the `KILN_*` overrides, which at this stage outrank the matching keys in the file. See [/en/reference/env/](/en/reference/env/).
  </Step>
...
In every mode the adjustment is one-way: adaptation lowers what you configured and never raises a smaller value. Per-profile budgets, the CPU ceiling rules, and the fixed Lite profile are covered in [/en/guide/variants/](/en/guide/variants/).
...
Changing a username or password from the admin surface writes an override row keyed by the original config username; the config file itself is never rewritten. The full authentication model is in [/en/guide/auth/](/en/guide/auth/).

## `[security]`

| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `play_require_auth` | bool | `true` | Whether playback endpoints require credentials. |
| `allowed_hosts` | array of string | `[]` | Extra hosts allowed for outbound fetches. Hosts from upstream `base_url` and channel `source_url` are added automatically, so this only needs hosts the config does not already imply. |
...
<Aside type="caution" title="play_require_auth can only be turned off from the environment">
  While defaults are being filled in, `play_require_auth` is forced back to `true` whenever `KILN_PLAY_OPEN` is unset. Setting `play_require_auth = false` in the file alone therefore has no effect: disabling playback authentication requires `KILN_PLAY_OPEN=1`, and that switch belongs in debugging environments only.
</Aside>

The outbound allowlist sits on top of a fixed guard that no configuration relaxes: link-local, unspecified, multicast, and cloud metadata addresses are always refused, and loopback or private addresses must appear in the allowlist to be reachable.
...
| `hls_list_size` | int | `8` | Segments retained in remuxed output playlists. Non-positive values fall back to the default. |
...
| `low_latency` | bool | `false` | Enables the low-latency output arguments. |
...
| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `enabled` | bool | `true` | Whether `/metrics` is served. It is forced to `true` while defaults are filled in, so writing `false` in the config has no effect. |
...
pprof uses its own listener and mux and is never mounted on the public server. Turn it on only while collecting CPU, heap, block, or mutex profiles. Diagnostic workflows live in [/en/guide/troubleshooting/](/en/guide/troubleshooting/).

## `[epg]`

| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `enabled` | bool | `false` | On the Lite variant, `true` refuses to start. The Full and Core variants do not read this key; whether EPG produces output depends on having at least one enabled source. |
...
Channel matching and how built-in sources are handled are covered in [/en/guide/epg/](/en/guide/epg/).

## `[[proxies]]`

| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `id` | string | Required | Route identifier, referenced by `egress.default`, `[[egress.rules]]`, `[[upstreams]]`, and `[[epg.sources]]`. `direct` is reserved and means no proxy. |
| `name` | string | `""` | Display name. |
...
| `docker_proxy_host` | string | `"host.docker.internal"` | Hostname substituted when a container reaches a proxy on the host. |
...
Route selection, rewriting policies, and container scenarios are covered in [/en/guide/proxy/](/en/guide/proxy/).
...
| `base_url` | string | Required | Upstream base address. Must parse as an absolute URL; empty or malformed fails validation. Its hostname joins the outbound allowlist automatically. |
| `proxy` | string | `""` | Proxy dedicated to this upstream. When set it must parse as an absolute URL — note this is a full URL, not a `[[proxies]]` id. |
...
Every string field is limited to 512 characters and must contain no control characters, otherwise validation fails. With the `ffmpeg` engine, a `subtitles.mode` of `prefer` or `only` fails validation outright — only `auto` and `off` are available there. Channel fields and selection in practice are covered in [/en/guide/channels/](/en/guide/channels/).
