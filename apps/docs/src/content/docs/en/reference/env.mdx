...
| `KILN_PUBLIC_BASE_URL` | Sets the startup value of `server.public_base_url` | Absolute URL; a trailing slash is trimmed | Above the config file, below an existing database setting |
...
### KILN_PUBLIC_BASE_URL does not replace an existing database setting

Core and Full write this startup value to SQLite on first start. Once `public_base_url` exists in the database, later environment and config-file values do not overwrite it. Change an existing instance in Settings or with `PUT /v1/admin/settings`. Lite has no SQLite database and continues to use environment above config file.
