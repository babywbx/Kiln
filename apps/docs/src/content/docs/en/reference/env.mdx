...
### KILN_PLAY_OPEN is the only switch for playback auth

The decision happens in two steps. First, `1`, `true`, or `TRUE` sets `security.play_require_auth` to `false`, while `0`, `false`, or `FALSE` sets it to `true`. Then, while defaults are filled in, an unset variable forces `play_require_auth` back to `true` unconditionally. The consequence is that `play_require_auth = false` in the config file does nothing on its own: disabling playback authentication requires an explicit `KILN_PLAY_OPEN=1`, and that switch belongs in debugging environments only.
