...
```json
{ "name": "kiln", "version": "1.1.1", "commit": "dev", "admin": "/admin" }
```
...
This endpoint follows `security.play_require_auth`. When authorization is required, it accepts a session or preview JWT through `Authorization: Bearer` or `?token=`, filters scoped credentials to their channels, and puts the same token into the generated `/v1/play/` URLs. An admin API token is not accepted. When authorization is disabled, the endpoint is public and generates token-free URLs for every enabled channel. With active EPG sources, the playlist header carries `x-tvg-url` pointing at `/v1/epg.xml.gz`.

## Session-only endpoints

These accept a login session only. Admin API tokens are denied with `session_required`; this boundary keeps a token from changing account credentials or managing other tokens.
...
| --- | --- | --- | --- |
| `GET` | `/v1/play/{id}/index.m3u8` | Depends on `play_require_auth` | Channel entry playlist |
| `GET` | `/v1/play/{id}/live/{file}` | Depends on `play_require_auth` | Locally packaged output: media playlists, segments, CMAF parts |
| `GET` | `/v1/play/{id}/u/{upstream}` | Depends on `play_require_auth` | Signed upstream fetch proxy |
...
- `u/{upstream}` proxies an upstream fetch. The target URL is encoded and carries an HMAC `sig`; a mismatch returns 403, and the target host must also pass the egress allowlist. Playlist responses are read in full and rewritten further. Other media requests forward only `Range` and `If-Range`; a 206 preserves `Content-Range`, `Accept-Ranges`, `Content-Length`, `ETag`, and `Last-Modified`. Upstream 404, 410, and 416 statuses pass through with an empty body, with only `Content-Range` retained for 416. Other upstream errors become a controlled 502 without exposing the upstream body or secret response headers.
...
```json
{
  ...
  "effective_user_agent": "Kiln/1.1.1",
  "revision": 3,
  "updated_at": 0
}
```
...
The read returns the config-file values `listen`, `cors_origins`, `public_hosts`, `play_require_auth`, and `tls_listen`, plus the runtime-editable `public_base_url`, `access_log_retention_days`, and `tls_enabled`. It also reports whether split listening is configured, the certificate source, path, hosts, expiry or error, and the current `revision`.
...
A `target` of `public` uses the built-in public HTTPS probe. `source` and `custom` require `url`, and the destination must be a public address. With no `target` and no `url`, the built-in probe is used. `proxy_url` tests an unsaved route directly, and `draft` tests an entire unsaved configuration. A valid test request returns 200 even when connectivity fails and reports the outcome in the response fields; authentication, request validation, target safety validation, and server errors still use their corresponding non-200 statuses:
...
On failure `ok` is `false` and `outcome` is one of `slow`, `blocked`, `dns`, `timeout`, `tls`, `proxy`, `proxy_auth`, `http_error`, or `network`, with an `error` description alongside. `ttfb_ms` and `sample_bytes` are present after an HTTP response is received; `throughput_kbps` is included only when the sample can support a throughput verdict. `slow` means the target was reachable but measured below `egress.min_test_kbps`.
...
| `GET` | `/v1/playlist.m3u` | Depends on `security.play_require_auth` |
| `GET` | `/v1/play/{id}/index.m3u8` | Depends on `security.play_require_auth` |
| `GET` | `/v1/play/{id}/live/{file}` | Depends on `security.play_require_auth` |
| `GET` | `/v1/play/{id}/u/{upstream}` | Depends on `security.play_require_auth` |

What differs:

- `/readyz` always reports ready and runs no compatibility-engine check.
- `/v1/playlist.m3u` and `/v1/play/` both follow `security.play_require_auth`; when enabled, they accept either an `Authorization: Bearer` header or a `?token=` query parameter.
