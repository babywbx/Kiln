---
title: API reference
description: Credential model, error contract, rate limits, and every registered route in the Kiln HTTP API.
sidebar:
  order: 4
---

The full Kiln binary registers 69 routes, split across four credential classes: public endpoints, session-or-token endpoints, session-only endpoints, and playback endpoints. This page lists all of them, including the admin-console-only routes the README overview table folds away.
...
`POST /v1/auth/login` exchanges a username and password for a JWT. Tokens are Ed25519-signed and carry issuer, audience, `exp`, `iat`, `nbf`, and `jti`, validated with 30 seconds of clock leeway. Lifetime comes from `auth.token_ttl` and defaults to 24 hours. The token carries `role` and `channels` and is bound to the account's `auth_revision`, so changing login credentials invalidates every previously issued token.
...
Playback keys are created under Playback Access Control in the admin console. They can be scoped to all channels or an explicit channel ID list, given an expiry, and revoked at any time. Every use through `/p/{token}/` is recorded in the playback access log. The first 10 characters serve as the display prefix, and both the access log and the request log keep only that prefix.
...
Every response carries `X-Request-ID`, echoed from the request when present and generated otherwise. Responses also set `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, `X-Frame-Options: DENY`, a `Content-Security-Policy`, and `Cache-Control: no-store, no-cache, must-revalidate`.
...
| Endpoint | `If-Match` |
| --- | --- |
| `PUT` and `DELETE` on `/v1/admin/settings`, `/v1/admin/egress`, and its `proxies` and `rules` sub-resources | Required; missing means 409 |
...
| `PUT` | `/v1/admin/api-tokens/{id}` | Update name, note, permissions, enabled state, expiry |
...
Body: `{ "name": "ci", "note": "", "scopes": ["read"], "expires_in_sec": 0 }`. `name` is required, at least one permission must be granted, and `expires_in_sec` of 0 means no expiry, capped at 10 years. Returns 201:
...
The create body is `{"name": "...", "note": "...", "channel_ids": ["demo-hls"], "expires_in_sec": 0}`. An empty `channel_ids` means all channels, and `expires_in_sec` of 0 means no expiry, capped at 10 years. Returns 201:
...
A proxy profile needs an `id` and a `url`. Schemes are limited to `http`, `https`, `socks5`, and `socks5h`, and the `id` cannot be the reserved value `direct`. Rule `kind` is one of `host_suffix`, `host_exact`, `channel`, `host_regex`, or `url_regex`; regex kinds are compiled during validation. Rules cannot reference a missing or disabled profile. `playlist_policy` is `rewrite`, `passthrough`, or `auto`. Deleting a profile also drops the rules that reference it and resets the default to `direct`.
