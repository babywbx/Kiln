...
Kiln is a single executable. There is no daemon wrapper and no hidden subprocess supervisor. The command line is four flags and one platform-specific subcommand.
...
### Runtime behaviour under the service manager
...
<CardGrid>
  <Card title="Deployment" icon="ph:download">
    Binary, container, and install-script paths in [Getting started](/en/start/).
  </Card>
  <Card title="Configuration" icon="ph:sliders">
    Every config section and value in [Configuration](/en/reference/config/).
  </Card>
  <Card title="Operations" icon="ph:gear">
    Process supervision, logging, and upgrades in [Operations](/en/guide/operations/).
  </Card>
</CardGrid>
