...
It uses the system trust store and performs normal certificate validation, with no insecure or custom-CA flag. The Lite image uses this command against HTTP. Core and Full use `wget` to try HTTPS first and fall back to HTTP. A manual HTTPS probe fails until an auto-generated certificate is trusted.
