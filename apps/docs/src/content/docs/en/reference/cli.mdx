...
```text
kiln version=1.1.1 commit=dev built_at=unknown variant=full
```

`variant` identifies the build as `full` or `lite`. Release artifacts receive `version`, `commit`, and `built_at` through linker flags; a direct source run uses `dev` and `unknown` for the latter two fields.
...
It performs normal certificate validation, with no insecure or custom-CA flag. The Lite image uses this command against HTTP. Core and Full use `wget` to try HTTPS first and fall back to HTTP. An auto-generated self-signed certificate fails default validation, so a manual HTTPS probe fails.
...
- The config is validated before startup. Failing any of the following exits with status 1 and logs `lite config rejected`:
  ...
  - `server.tls_enabled` must be `false`, and `server.tls_listen` plus both TLS certificate paths must be empty.
