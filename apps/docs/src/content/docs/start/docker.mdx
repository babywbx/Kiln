---
title: Docker 部署
description: 三个镜像变体、拉取方式、最小运行命令与容器内的资源行为。
sidebar:
  order: 3
---

Kiln 发布三个镜像变体，边界明确，共用同一套配置模型和同一份原生媒体模块。选哪一个取决于是否需要 FFmpeg 兼容回退，以及是否需要数据库与管理控制台。

## 三个变体

| 镜像 | 能力 | 默认 packager | FFmpeg | 定位 |
| --- | --- | --- | --- | --- |
| `lite` | 配置、登录、M3U、播放 | `native` | 不包含 | 基于 scratch 的极简运行时，3.8 MB，无数据库 |
| `core` | 完整 | `native` | 不包含 | 完整管理与观测，纯原生媒体路径 |
| `full` | 完整 | `auto` | 内置 | 优先原生，必要时回退到 FFmpeg |
...
| 标签形态 | 含义 |
| --- | --- |
| `:1.0.0`、`:1.0`、`:1`、`:latest` | full 变体的精确版本、次版本、主版本与最新版 |
| `:1.0.0-core`、`:1.0-core`、`:1-core`、`:core` | core 变体的同一组标签 |
| `:1.0.0-lite`、`:1.0-lite`、`:1-lite`、`:lite` | lite 变体的同一组标签 |

预发布版本只推出带完整版本号的那一个标签，不会移动 `latest`、`core`、`lite` 以及主次版本标签。core 覆盖 `linux/amd64`、`linux/arm64`、`linux/arm/v7` 与 `linux/arm/v6`，full 与 lite 覆盖 `linux/amd64` 与 `linux/arm64`。三个变体在发布时都会生成构建溯源证明并推送到镜像仓库。
...
<Aside type="caution" title="纯原生镜像不要指定 ffmpeg 引擎">
  core 与 lite 不含 ffmpeg。若配置里有 DASH 频道显式落到 `ffmpeg` 引擎，`/readyz` 会返回 503 并说明兼容引擎不可用，容器不会进入就绪状态。要么改用 `native`，要么换 full 镜像。
</Aside>
...
自适应只下压、不上调：配置里已经写得更低的值不会被抬高。启动日志会打印探测结果、生效档位和全部预算，用来核对容器是否落在预期档位。
