...
lite 不创建 SQLite，公开接口只有 `/healthz`、`/readyz`、`/v1/auth/login`、`/v1/playlist.m3u` 和 `/v1/play/*`。配置里出现 `auto` 或 `ffmpeg` packager、EPG、OTLP、pprof 或 TLS 时，进程会在启动时直接拒绝而不是静默忽略。变体之间的能力差异见 [镜像变体对比](/guide/variants/)。
...
| 标签格式 | 含义 |
| --- | --- |
| `:1.1.1-lite`、`:1.1-lite`、`:1-lite`、`:lite` | lite 变体的精确版本、次版本、主版本与最新版 |
| `:1.1.1-core`、`:1.1-core`、`:1-core`、`:core` | core 变体的同一组标签 |
| `:1.1.1`、`:1.1`、`:1`、`:latest` | full 变体的同一组标签，`:latest` 即 full 的最新版 |
...
services:
  kiln:
    ...
    ports:
      - "8080:8080"
      # server.tls_listen = "0.0.0.0:8443" 时再发布这一项：
      # - "8443:8443"
    environment:
...
几处值得留意：日志颜色设为 `never`，`docker logs` 里就不会混入 ANSI 转义；`KILN_PUBLIC_BASE_URL` 提供播放列表对外地址的启动值，但 Core 与 Full 的已有数据卷会优先使用数据库设置，修改现有实例请走设置页或 `/v1/admin/settings`；上游如果是同一个 Compose 网络里的另一个服务，把 `base_url` 写成 `http://service-name:port` 并让两者共享网络即可。分离 HTTPS 还要发布第二个端口，见 [HTTPS 与分离监听](/guide/https/)。

## 健康检查

`/healthz` 与 `/readyz` 都不需要凭据。`/healthz` 表示进程活着，`/readyz` 会额外检查兼容引擎的可用性，只有当配置里存在落到 ffmpeg 引擎的 DASH 频道而 ffmpeg 不可用时才返回 503。

三个镜像都自带 `HEALTHCHECK`：间隔 30 秒、超时 3 秒、启动宽限 10 秒、连续 3 次失败判定为 unhealthy。core 与 full 用 `wget --no-check-certificate` 先探测 HTTPS，再回退 HTTP；lite 基于 scratch，没有外部命令，使用二进制自带的 `-healthcheck` 探测 HTTP。这个子命令也可以在容器外单独使用：

```bash
docker exec kiln /usr/local/bin/kiln -healthcheck http://127.0.0.1:8080/healthz
```

`-healthcheck` 会正常校验证书。自动生成的自签名证书无法通过默认校验，因此手工 HTTPS 探测会失败；分离监听优先探测明文健康端口。详见 [HTTPS 与分离监听](/guide/https/)。
...
`data_dir` 是唯一需要持久化的目录，镜像里默认是 `/var/lib/kiln/data`。core 与 full 在其中放 SQLite 数据库 `kiln.db`、自动生成的会话密钥 `auth/ed25519.pem`，以及磁盘 EPG 模式下的 `epg/epg.db`。挂载点必须对 uid/gid `999` 可写，否则首次启动会失败。
