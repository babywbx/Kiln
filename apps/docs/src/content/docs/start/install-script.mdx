...
重复运行脚本就是升级。版本比较覆盖 SemVer 核心版本和预发布标识符，因此 `1.1.1-rc.1` 会被正确判定为早于 `1.1.1`。

```bash
curl -fsSL https://raw.githubusercontent.com/babywbx/Kiln/main/install.sh | sh -s -- --lang zh --version 1.1.1
```
