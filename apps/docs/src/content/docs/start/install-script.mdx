---
title: 安装脚本
description: 一行命令安装 Kiln：镜像加速、静默安装、systemd 服务与卸载。
sidebar:
  order: 2
---

Linux 与 macOS 用一条命令装好，重复运行即升级。脚本只做四件事：探测平台、选择可用的下载源、校验 `SHA256SUMS`、原子替换二进制。默认不需要 sudo，装到哪里、要做什么都会先展示，确认后才动手。
...
<Steps>
  ...
  <Step title="下载、校验、原子替换">
    下载压缩包，比对 `SHA256SUMS`，解包后先写入同目录下的暂存文件并做一次启动冒烟，通过后才原子替换。
  </Step>
</Steps>
...
| `--version <v>` | 固定版本，写不写前导 `v` 都行；必须是合法 semver，可带预发布标识符 | 最新 Release |
...
重复运行脚本就是升级。版本比较走完整 semver 规则，包含预发布标识符的逐段比较，因此 `1.0.0-rc.1` 会被正确判定为早于 `1.0.0`。
