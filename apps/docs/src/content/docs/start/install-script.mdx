...
拉取脚本本身失败时，请先通过可信的 HTTPS 下载源保存到本地再执行；`--mirror` 只控制脚本后续下载发行包时使用的来源。
...
<Steps>
  ...
  <Step title="解析版本与下载源">
    未指定 `--version` 时解析最新 Release。未显式指定镜像时，选择产物下载源会先探测主发布源，直连失败后才探测镜像。
  </Step>
  <Step title="展示计划并确认">
...
| `--mirror <base>` | 手动指定 HTTPS 下载镜像，跳过探测；地址不得含空白字符，结尾多余的斜杠会被去掉 | 自动探测 |
...
脚本内置多个 HTTPS 下载镜像，具体列表以脚本为准。

未指定 `--mirror` 时，脚本先单独探测主发布源，超时 3 秒。只有直连失败且未指定 `--no-mirror` 时，才并行探测内置镜像，并按内置顺序选取第一个可用源。输出会明确标注是否使用镜像。

`--mirror` 会跳过整个探测过程，直接使用给定地址；`--no-mirror` 则把候选集合缩减为只有直连，探测不通就报错退出，不做任何回退。

默认解析最新版本时，脚本先试主发布源的 Release 跳转地址与 API；仍拿不到且允许镜像时，再按顺序逐个尝试内置镜像。

<Aside type="caution" title="校验和只取自主发布源">
  无论压缩包从哪里下载，`SHA256SUMS` 都只从主发布源获取，超时 10 秒。取不到时脚本以 3 退出，不会改用镜像提供的校验和，也不会触碰已有安装。
</Aside>
...
重复运行脚本就是升级。版本比较覆盖 SemVer 核心版本和预发布标识符，因此 `1.1.0-rc.1` 会被正确判定为早于 `1.1.0`。

```bash
curl -fsSL https://raw.githubusercontent.com/babywbx/Kiln/main/install.sh | sh -s -- --lang zh --version 1.1.0
```
...
下载完成后，脚本从主发布源获取 `SHA256SUMS`，超时 10 秒；取不到时以 3 退出，不回退到镜像。随后从中取出本次产物对应的一行做比对；不匹配就立即丢弃下载的文件并以 4 退出，不触碰已有安装。
