---
title: 配置参考
description: Kiln 配置文件的全部配置节与键，含类型、默认值、约束与环境变量等价项。
sidebar:
  order: 2
---

本页按配置节逐键列出 Kiln 的配置文件格式。默认值一栏写的是源码中实际生效的值，可能与示例文件里写出的值不同：示例文件是一份可用的起点，不是默认值清单。
...
校验通过之后，资源自适应才开始工作：它会依据探测到的内存与 CPU 上限，**只下压不上调**地覆盖 `server.memory_limit_mb`、`packager.inflight_bytes`、`packager.max_segment_bytes`、`packager.start_segments`、`packager.prefetch_segments`、`epg.max_refresh_concurrency` 与 `epg.max_source_bytes`。启动日志会打印生效后的实际值。
...
三种模式下的下压都是单向的：自适应只会把你配置的数值调小，不会把更小的数值调大。档位对应的具体预算、CPU 上限规则与 Lite 变体的固定档位见 [/guide/variants/](/guide/variants/)。

## `[logging]`

| 键 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `level` | string | `"info"` | 日志级别，识别 `debug`、`info`、`warn`、`error` 及其常见别名，无法识别的值回落到 `info`。等价环境变量 `KILN_LOG_LEVEL`。 |
...
| 键 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `play_require_auth` | bool | `true` | 播放接口是否要求鉴权。 |
...
<Aside type="caution" title="play_require_auth 只能被环境变量关闭">
  填充默认值时，只要 `KILN_PLAY_OPEN` 未设置，`play_require_auth` 就会被强制置回 `true`。也就是说，仅在配置文件里写 `play_require_auth = false` 不会生效，关闭播放鉴权必须设置 `KILN_PLAY_OPEN=1`，且该开关只适合调试环境。
</Aside>
...
| 键 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `enabled` | bool | `true` | 是否暴露 `/metrics`。填充默认值时会被无条件置为 `true`，在配置里写 `false` 不会生效。 |
...
| 键 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `enabled` | bool | `false` | 在 Lite 变体中，置为 `true` 会直接拒绝启动。Full 与 Core 变体不读取此键，EPG 是否有输出取决于是否存在已启用的源。 |
...
| 键 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `id` | string | 必填 | 线路标识，被 `egress.default`、`[[egress.rules]]`、`[[upstreams]]` 与 `[[epg.sources]]` 引用。`direct` 是保留标识，代表直连。 |
| `name` | string | `""` | 展示名称。 |
| `url` | string | 必填 | 代理地址，协议限 `http`、`https`、`socks5`、`socks5h`，例如 `http://127.0.0.1:7890` 或 `socks5h://127.0.0.1:7891`。 |
| `disabled` | bool | `false` | 是否停用。停用的线路不参与路由，引用它的决策会回落到直连。 |
...
| `proxy` | string | `""` | 该上游专用的代理地址，非空时必须是可解析的绝对 URL。注意这里写的是完整 URL，不是 `[[proxies]]` 的 `id`。 |
...
| `epg_name` | string | `""` | 按名称匹配 XMLTV 频道时使用的名称，留空则回落到 `title`。 |
...
| `audio.preferred_languages` | array of string | `[]` | 音轨语言优先级，留空时回落到频道级 `preferred_audio_languages`。 |
...
所有字符串字段长度不得超过 512，且不得包含控制字符，否则校验失败。使用 `ffmpeg` 引擎时，`subtitles.mode` 取 `prefer` 或 `only` 会直接校验失败，只能用 `auto` 或 `off`。频道字段与选轨的实操说明见 [/guide/channels/](/guide/channels/)。
