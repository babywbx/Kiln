...
`[[channels]]` 只在频道表为空时作为首次启动的种子写入 `server.data_dir` 下的 SQLite，之后管理端的改动以数据库为准，再改配置文件不会覆盖它们。`[[proxies]]` 与 `[[egress.rules]]` 则**每次启动都会按 ID 从配置文件同步进数据库**：同 ID 的配置文件内容优先，只在控制台创建、配置文件里没有同 ID 的线路与规则不会被删除。配置文件没有携带凭据，且库中同一代理的 scheme 与 host 都没有改变时，已有凭据会被保留。`egress.default`、`egress.playlist_policy` 与 `egress.docker_proxy_host` 同样每次同步；由于配置加载会填入 `direct`、`rewrite` 与 `host.docker.internal`，即使文件省略字段也会用默认值覆盖数据库。`server.public_base_url` 仍以「不存在才写入」的方式落库，只在首次生效。`[[upstreams]]`、`[auth]` 与其余全局键始终以配置文件为准，其中 `[[auth.users]]` 的用户名与密码可以被管理端改写并存入数据库覆盖表。

## `[server]`

| 键 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `listen` | string | `"0.0.0.0:8080"` | 主监听地址。TLS 关闭时提供 HTTP；TLS 开启且未配置 `tls_listen` 时提供 HTTPS；分离监听时保留为受限 HTTP。等价环境变量 `KILN_LISTEN`。 |
| `public_base_url` | string | `"http://127.0.0.1:8080"` | 对外可访问的基础 URL，用于生成播放地址等外部链接，尾部斜杠会被去掉。Core 与 Full 只在数据库没有该设置时用配置文件或 `KILN_PUBLIC_BASE_URL` 初始化；Lite 始终使用启动配置。 |
| `data_dir` | string | `"./data"` | 数据目录，存放 SQLite、自动生成的签名密钥与默认的 EPG 数据库，启动时以 `0750` 创建。等价环境变量 `KILN_DATA_DIR`。 |
...
| `tls_enabled` | bool | `false` | 以 HTTPS 提供服务。控制台里的开关会写入数据库并覆盖这里的取值，重启后生效。 |
| `tls_listen` | string | `""` | 可选。启用 HTTPS 时，设了值就在这个地址增加一个提供全部功能的 TLS 监听器，`listen` 保持明文，只提供公开的播放与探针接口。端口必须是数字，且不能与 `listen` 重叠；通配地址与同端口的具体地址也算重叠。留空则 TLS 作用于唯一的监听地址。 |
| `tls_cert_file` | string | `""` | PEM 证书路径，必须与 `tls_key_file` 同时设置。留空则在 `data_dir/tls/` 下自动签发并复用自签名证书。 |
| `tls_key_file` | string | `""` | PEM 私钥路径，必须与 `tls_cert_file` 同时设置。 |

TLS 仅在 Core 与 Full 中可用。Lite 遇到 `tls_enabled = true`、配置了 `tls_listen` 或任一证书路径时会拒绝启动，不会静默改用 HTTP。监听模式、受限 HTTP 路由、证书信任与 Docker 端口见 [HTTPS 与分离监听](/guide/https/)。
...
| `upstream_rsa_key_exchange` | bool | `true` | 抓取上游时是否额外提供 RSA 密钥交换套件。Go 1.22 起默认不再提供这类套件，而部分 CDN 边缘节点只支持它们，握手会直接失败。开启后仍优先协商具备前向保密的套件，仅在对端别无选择时回落。设为 `false` 可强制只用前向保密套件，代价是这些节点无法访问。该设置只影响向上游发起的连接，不影响本服务对外提供的 HTTPS。 |
...
| `fetch_stall_sec` | int | `30` | 上游拉流的无进展超时秒数：连续这么久没有收到新字节才判失败，非正数按默认值处理。它计的是进度而不是请求总时长，所以几兆的 4K 分片只要还在传就不会被中断。资源自适应不会改动它。 |
| `inflight_bytes` | int64 | `100663296` | 全部频道共用的分片内存预算字节数，非正数按默认值处理。资源自适应可能下调。 |
| `rendition_idle_sec` | int | `30` | 按需拉流的闲置阈值秒数：主视频轨始终拉取，其余轨道这么久没有任何播放请求就暂停回源，再次被请求时自动跳回直播最前沿并以不连续点续播。`0` 按默认值处理，负数关闭按需拉流、恢复全量拉取。上限 86400。 |
| `reanchor_sec` | int | `30` | 单条轨道无进展重锚的阈值秒数：有新分片可取却持续这么久没有发布成功，就跳到直播最前沿重新开始。`0` 按默认值处理，负数关闭无进展重锚，时间线回退触发的重锚不受影响。上限 3600。 |
| `segment_fetch_cap_sec` | int | `0` | 直播分片单次下载的时长上限秒数，超时会换新连接重试一次，防止慢而不死的连接冻结整条流水线。`0` 表示自动取分片时长的两倍且不低于 20 秒，负数关闭上限。仅作用于直播，点播不受限。上限 3600。 |
| `manifest_fetch_retries` | int | `2` | 清单拉取失败后的重试次数，只对网络类瞬时错误退避重试，HTTP 状态码等永久错误立即失败。`0` 按默认值处理，负数关闭重试。上限 10。 |
...
| `cache_dir` | string | `"{data_dir}/epg"` | 节目单库所在目录，库文件为其中的 `epg.db`。写成 `memory` 或 `:memory:` 时改用内存库，其余值按目录路径处理。 |
...
| `max_source_bytes` | int64 | `67108864` | 单个源解压后允许的字节上限，流式读取过程中超出即拒绝，非正数按默认值处理。资源自适应可能下调。 |
...
| `name` | string | `""` | 展示名称。 |
| `url` | string | 必填 | 代理地址，协议限 `http`、`https`、`socks5`、`socks5h`，例如 `http://127.0.0.1:7890` 或 `socks5h://127.0.0.1:7891`。所有 scheme 的目标都由 Kiln 本机解析并固定 IP；`socks5h` 仅作为兼容写法保留。明文 HTTP 上游不能经 HTTP(S) 转发代理，需使用 HTTPS 上游或 SOCKS5。 |
| `disabled` | bool | `false` | 是否停用。默认出口或已启用规则引用停用线路时，配置校验或管理端保存会失败，不会退回直连。 |

## `[egress]`

| 键 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `default` | string | `"direct"` | 未命中任何规则时使用的线路，必须是 `direct` 或某个已定义且启用的 `[[proxies]].id`，否则校验失败。 |
| `playlist_policy` | string | `"rewrite"` | 播放列表地址改写策略，取值 `rewrite`、`passthrough`、`auto`，其它值校验失败。`rewrite` 始终改写为经由 Kiln 的地址，`passthrough` 始终保留原始地址。`auto` 在实际走代理时改写；频道配置了自定义 User-Agent 或有效上游请求头时也会全部改写，避免播放器直连时丢失这些配置。 |
...
| `min_test_kbps` | int | `1000` | 连通性测试的吞吐门槛。测试会读取至多 4 MiB 或 5 秒；采样窗口耗尽时按实际字节测速，低于该值即判定为「可达但不足以承载播放」。响应在 256 KiB 前提前结束时不推断带宽，只报连通结果。负数关闭该判定。 |
| `trust_proxy_dns` | bool | `false` | 让代理解析目标域名，Kiln 不再本机解析并固定拨号。仅在宿主机运行 TUN 或 fake-IP DNS、任何进程都拿不到真实地址时开启。开启后 Kiln 无法校验目标 IP，只保留 scheme 与主机名层面的检查，因此只应指向可信代理。仅由配置文件控制，控制台无法修改。 |
...
| `pattern` | string | `""` | 匹配内容，已启用规则不能为空。正则类按 Go 正则语法编译，编译失败会让配置校验或管理端保存失败。 |
| `proxy` | string | 必填 | 命中后使用的线路，必须是 `direct` 或已定义且启用的 `[[proxies]].id`，为空、未知或停用都会校验失败，不会退回直连。 |
...
| `upgrade_insecure_redirects` | bool | `false` | 兼容只提供 HTTPS、却把重定向目标错误写成 `http` 的源站。开启后，仅将公网主机默认端口的重定向升级为 `https`；私网白名单主机与非默认端口保持原样。 |
...
| `upgrade_insecure_redirects` | bool | `false` | 针对该频道开启不安全重定向升级。直接使用 `source_url` 的频道需要在此开启；引用 `upstream` 的频道也会继承上游设置。 |
