...
`variant` 区分构建变体，完整版为 `full`，Lite 版为 `lite`。正式产物的 `version`、`commit` 与 `built_at` 由构建时的链接参数注入；从源码直接运行时，`commit` 与 `built_at` 分别为 `dev` 和 `unknown`。
...
它会正常校验证书，没有跳过校验或自定义 CA 参数。Lite 镜像直接用这条命令探测 HTTP；Core 与 Full 则用 `wget` 先探测 HTTPS、失败后回退 HTTP。自动生成的自签名证书无法通过默认校验，因此手工 HTTPS 探测会失败。
...
- 启动前会校验配置，不满足以下任一条件即拒绝启动，以状态码 1 退出并记录 `lite config rejected`：
  ...
  - `server.tls_enabled` 必须为 `false`，`server.tls_listen` 与两个 TLS 证书路径都必须为空。
