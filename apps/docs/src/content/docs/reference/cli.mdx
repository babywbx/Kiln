---
title: 命令行
description: kiln 二进制的全部启动参数、Windows 服务子命令、Lite 差异与辅助脚本用法。
sidebar:
  order: 5
---

Kiln 是单个可执行文件，没有守护进程包装层，没有隐藏的子进程管理器。命令行只有四个参数和一个平台相关的子命令。
...
<CardGrid>
  <Card title="部署方式" icon="ph:download">
    二进制、容器与安装脚本三种方式，见 [入门](/start/)。
  </Card>
  <Card title="配置参考" icon="ph:sliders">
    配置文件的全部段落与取值，见 [配置参考](/reference/config/)。
  </Card>
  <Card title="运维" icon="ph:gear">
    进程托管、日志与升级流程，见 [运维](/guide/operations/)。
  </Card>
</CardGrid>
