...
这条接口跟随 `security.play_require_auth`。需要鉴权时，它通过 `Authorization: Bearer` 或 `?token=` 接受会话或预览 JWT，按凭据的频道范围过滤，并把同一令牌写进生成的 `/v1/play/` 地址；管理员 API 令牌不能使用。关闭鉴权时，接口公开，并为全部已启用频道生成不带令牌的地址。存在已启用的 EPG 源时，播放列表头部带 `x-tvg-url` 指向 `/v1/epg.xml.gz`。

## 仅会话端点

只接受登录会话。管理员 API 令牌访问时以 `session_required` 拒绝，这道边界防止令牌修改账号凭据或管理其它令牌。
...
| 方法 | 路径 | 凭据 | 说明 |
| --- | --- | --- | --- |
| `GET` | `/v1/play/{id}/index.m3u8` | 由 `play_require_auth` 决定 | 频道主播放列表 |
| `GET` | `/v1/play/{id}/live/{file}` | 由 `play_require_auth` 决定 | 本地封装产物：媒体播放列表、分片、CMAF part |
| `GET` | `/v1/play/{id}/u/{upstream}` | 由 `play_require_auth` 决定 | 已签名的上游回源代理 |
...
- `u/{upstream}` 是上游回源代理。目标地址经过编码并附带 HMAC 签名 `sig`，签名不匹配返回 403；目标主机还要通过出站白名单校验。返回内容为播放列表时会完整读取并继续改写。其它媒体请求只向上游转发 `Range` 与 `If-Range`，206 响应保留 `Content-Range`、`Accept-Ranges`、`Content-Length`、`ETag` 与 `Last-Modified`。上游 404、410、416 以空正文透传，其中 416 只保留 `Content-Range`；其它上游错误返回受控的 502，不透传错误正文或秘密响应头。
...
读取返回配置文件中的 `listen`、`cors_origins`、`public_hosts`、`play_require_auth` 与 `tls_listen`，加上可在运行时修改的 `public_base_url`、`access_log_retention_days` 与 `tls_enabled`。响应还会给出是否配置了分离监听、证书来源、路径、覆盖主机、到期时间或错误，以及当前 `revision`。

写入的请求体为 `{"public_base_url": "https://kiln.example.com", "access_log_retention_days": "30", "tls_enabled": true}`。前两个字段为字符串，保留天数取值范围为 1 至 3650，TLS 变更在重启后生效。`tls_enabled` 可省略，省略时保留当前值。必须携带 `If-Match`，缺失返回 409。
...
`target` 取 `public` 时使用内置公网 HTTPS 探测地址，取 `source` 或 `custom` 时必须提供 `url` 且目标必须是公网地址，缺省且没有 `url` 时按内置地址处理。`proxy_url` 可以直接测试一条尚未保存的线路，`draft` 可以整体测试一份尚未保存的配置草稿。有效测试请求即使连通性失败也返回 200，并在结果字段中表达；鉴权、请求校验、目标安全校验与服务端错误仍返回对应的非 200 状态：
...
失败时 `ok` 为 `false`，`outcome` 取值为 `slow`、`blocked`、`dns`、`timeout`、`tls`、`proxy`、`proxy_auth`、`http_error` 或 `network`，并附 `error` 描述。`ttfb_ms` 与 `sample_bytes` 在成功收到 HTTP 响应后给出；只有样本足以判断吞吐时才有 `throughput_kbps`。`slow` 表示目标可达，但实测吞吐低于 `egress.min_test_kbps`。
...
| `GET` | `/v1/playlist.m3u` | 由 `security.play_require_auth` 决定 |
| `GET` | `/v1/play/{id}/index.m3u8` | 由 `security.play_require_auth` 决定 |
| `GET` | `/v1/play/{id}/live/{file}` | 由 `security.play_require_auth` 决定 |
| `GET` | `/v1/play/{id}/u/{upstream}` | 由 `security.play_require_auth` 决定 |

差异要点：

- `/readyz` 恒定返回就绪，不做兼容引擎检查。
- `/v1/playlist.m3u` 与 `/v1/play/` 都受 `security.play_require_auth` 控制，开启时凭据可用 `Authorization: Bearer` 头或 `?token=` 查询参数。
- 错误响应格式与完整版一致。
...
<CardGrid>
  ...
  <Card title="配置项" icon="ph:sliders" href="/reference/config/">
    `auth`、`security`、`egress` 等段落的取值。
  </Card>
