...
video-main.m3u8
`
	out := string(rewriteLocalPlaylist([]byte(in), "/v1/play/demo/live/", "t0k"))

	for _, want := range []string{
		`URI="/v1/play/demo/live/audio-main.m3u8?token=t0k"`,
		"/v1/play/demo/live/video-main.m3u8?token=t0k",
	} {
...
`
	out := string(rewriteLocalPlaylist([]byte(in), "/v1/play/demo/live/", ""))
	for _, want := range []string{
		`#EXT-X-MAP:URI="/v1/play/demo/live/video-main-init.mp4"`,
		"/v1/play/demo/live/video-main-000001.m4s",
	} {
		if !strings.Contains(out, want) {
			t.Errorf("missing %q in:\n%s", want, out)
		}
...
func TestRewriteLocalPlaylistOmitsQueryTokenForPathTokens(t *testing.T) {
	...
	out := string(rewriteLocalPlaylist([]byte(in), "/p/abc/play/demo/live/", "t0k"))

	if strings.Contains(out, "token=") {
		t.Errorf("path-token playlist should carry no query token:\n%s", out)
	}
	if !strings.Contains(out, `URI="/p/abc/play/demo/live/video-main-init.mp4"`) {
