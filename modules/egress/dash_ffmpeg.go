...
const (
	...
	minReadyEXTINF     = 0.4
	readyTimeoutLocal  = 28 * time.Second
	readyTimeoutRemote = 35 * time.Second
)
...
func startPackager(parent context.Context, opt DashOptions, log *slog.Logger, absWork, key string, att packAttempt) (*DashJob, error) {
	...
	if opt.Egress != nil {
		...
		if env := opt.Egress.EnvForFFmpeg(opt.SourceURL, opt.ChannelID, forDocker); len(env) > 0 {
...
func resolveMPD(ctx context.Context, opt DashOptions) (string, string, error) {
	...
	if opt.Egress != nil {
		...
		if hc, err := opt.Egress.ClientForChannel(d.ProxyID, opt.ChannelID, 30*time.Second); err == nil {
			client = hc
		}
	}
